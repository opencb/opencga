...
public class FindingsCollectionMigrationTask7645 extends MigrationTool {
    ...
    protected void run() throws Exception {
        ...
        List<Long> interpretationUids = new LinkedList<>();
        try (MongoCursor<Long> iterable = interpretationArchCol.distinct("uid", filter, Long.class).cursor()) {
            while (iterable.hasNext()) {
                interpretationUids.add(iterable.next());
            }
        }
        int interpretationsProcessed = 0;
        int totalInterpretations = interpretationUids.size();
        int totalVersionsProcessed = 0;
        logger.info("Found {} interpretations to migrate findings from.", totalInterpretations);

        for (Long interpretationUid : interpretationUids) {
            interpretationsProcessed++;
            ...
            // Fetch all versions for this interpretation at once (outside of transaction)
            List<Document> allVersions = new ArrayList<>();
            try (MongoCursor<Document> cursor = interpretationArchCol
                ...
                while (cursor.hasNext()) {
                    allVersions.add(cursor.next());
                }
            }

            logger.info("Processing interpretation UID: {} ({}/{}). Found {} versions.", interpretationUid, interpretationsProcessed,
                    totalInterpretations, allVersions.size());

            // Process each version in a separate, short transaction
            int versionCount = 0;
            for (Document interpretationDoc : allVersions) {
                versionCount++;
                ...
                try {
                    ...
                    // Process this version in a single short transaction
                    ...
                    if (versionCount % 10 == 0) {
                        logger.info("Processed {}/{} versions for interpretation UID: {}", versionCount, allVersions.size(), interpretationUid);
                    }
                } catch (Exception e) {
                    throw new MigrationException("Error processing version " + interpretationDoc.getInteger("version")
                            + " for interpretation UID: " + interpretationUid + ". Error: " + e.getMessage(), e);
                }
            }
        ...
        logger.info("Migration of interpretation archive collection completed. Total interpretations: {}, Total versions processed: {}",
                interpretationsProcessed, totalVersionsProcessed);
