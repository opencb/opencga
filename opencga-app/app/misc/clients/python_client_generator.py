...
class PythonClientGenerator(RestClientGenerator):
    ...
    def get_imports(self):
        ...
        if self.app == 'opencga':
            text += 'from pyopencga.rest_clients._parent_rest_clients import ' \
                    '_ParentRestClient\n\n'
        elif self.app == 'cellbase':
            text += 'from pycellbase.rest_clients._parent_rest_clients import ' \
                    '_ParentRestClient\n\n'
        return text
    ...
    def get_class_definition(self, category):
        ...
        if self.app == 'opencga':
            ...
            text.append('{}def __init__(self, session, configuration):'.format(
                ' ' * 4, self.get_category_name(category), category['path'])
            )
            text.append('{}super({}, self).__init__(session, configuration)'.format(
                ' ' * 8, self.categories[self.get_category_name(category)])
            )
        text.append('')
        return '\n'.join(text)
...
def _setup_argparse():
    ...
    parser.add_argument('app', help='app', choices=['cellbase', 'opencga'])
...
def main():
    ...
    client_generator = PythonClientGenerator(args.app, args.server_url, args.output_dir)
