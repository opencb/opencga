...
class RelatednessAnalysis:
    ...
    def set_relatedness_dir(self):
        output_relatedness_dir = create_output_dir(path_elements=[self.executor["output_parent_dir"], 'relatedness'])
    ...
    def relatedness_plink(self, filtered_vcf_fpath, plink_path="plink1.9"):
        ...
        # Sex file; FamilyID SampleID Sex
        sex_fpath = os.path.join(plink_dir, "sex.txt")
        LOGGER.info(f"Generating text file to update sex information: '{sex_fpath}'")
        with open(sex_fpath, 'w') as file:
            file.write(f'{self.executor["id_"]}\t{self.executor["father_id"]}\t1\n')
            file.write(f'{self.executor["id_"]}\t{self.executor["mother_id"]}\t2\n')
            file.write(f'{self.executor["id_"]}\t{self.executor["sample_id"]}\t{self.executor["sex"]}\n')

        # Parents file; FamilyID SampleID FatherID MotherID
        parents_fpath = os.path.join(plink_dir, "parents.txt")
        LOGGER.info(f"Generating text file to update parents information: '{parents_fpath}'")
        with open(parents_fpath, 'w') as file:
            file.write(f'{self.executor["id_"]}\t{self.executor["father_id"]}\t0\t0\n')
            file.write(f'{self.executor["id_"]}\t{self.executor["mother_id"]}\t0\t0\n')
            file.write(f'{self.executor["id_"]}\t{self.executor["sample_id"]}\t{self.executor["father_id"]}\t{self.executor["mother_id"]}\n')

        # Preparing PLINK commands
        plink_path = str(plink_path)
        files_prefix = self.executor["id_"] + "_plink_relatedness_results"
        plink_output_folder_files_prefix = os.path.join(plink_dir, files_prefix)
        plink_files_args = ["--vcf", str(filtered_vcf_fpath),
                      "--make-bed",
                      "--const-fid", self.executor["id_"],
                      "--chr", "1-22",
                      "--not-chr", "X,Y,MT",
                      "--allow-extra-chr",
                      "--snps-only",
                      ...
                      "--update-sex", sex_fpath,
                      "--update-parents", parents_fpath,
                      "--out", plink_output_folder_files_prefix]
        cmd_plink_files = [plink_path] +  plink_files_args
        LOGGER.info('Generating PLINK files')
        ...
        if plink_files[0] == 0:
            ...
            if os.path.isfile(plink_genome_fpath) == False:
                ...
                for index,parameter in enumerate(all_parameters):
                    if parameter.startswith('--'):
                        ...
                        if (all_parameters[index+1].startswith('--'))==False:
                            ...
                            value = ""
                        params[key] = value
                        self.relatedness.software = Software(name="PLINK", version="1.9", commit="", params=params)
                    else:
                        continue
                ...
                # self.relatedness.attributes = Attributes(cli=cli, files=files)
