...
class FamilyQCExecutor:
    ...
    def relatedness_results_data_model(method="PLINK/IBD"):
        ...
        if method == "PLINK/IBD":
            ...
            relatedness_json["software"]["name"] = "plink"
            relatedness_json["software"]["version"] = "1.9"
        else:
            LOGGER.info('Relatedness method to be used: "{}"'.format(method))
            relatedness_json["method"] = method
    ...
    def relatedness_inference(self, relatedness_thresholds_fpath, method, plink_genome_fpath):
        ...
        for index, line in enumerate(input_genome_file_fhand):
            ...
            LOGGER.debug(
                "Inferring family relationship between sample {} and sample {} ".format(str(genome_file_row_values[1]),
                                                                                        str(genome_file_row_values[3])))
            inference_groups = []
            ...
            LOGGER.info(
                "Family relationship inferred between sample {} and sample {} ".format(str(genome_file_row_values[1]),
                                                                                       str(genome_file_row_values[3])))
    ...
    def relatedness_report(self, relatedness_inference_results):
        ...
        for score_result in relatedness_inference_results["scores"]:
            ...
            validation_result = self.relatedness_validation(relatedness_results["scores"]["reportedRelationship"],
                                                            relatedness_results["scores"]["inferredRelationship"])
