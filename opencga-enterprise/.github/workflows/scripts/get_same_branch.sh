...
echo "::group::Calculating DEPENDENCIES_SHA"
if [ -d "./opencga-home" ]; then
    cd "./opencga-home" || exit 2
    DEPENDENCIES_SHA=${DEPENDENCIES_SHA}:$(git rev-parse HEAD)
    echo "Folder opencga-home already exists."
    cd - || exit 2
fi
...
function checkout(){
  local REPO=$1
   cd "${WORKSPACE}" || exit 2
  echo "Installing '$REPO' project from branch $BRANCH_NAME"
