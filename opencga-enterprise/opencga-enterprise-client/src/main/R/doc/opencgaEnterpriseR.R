...
#  ## Install opencgaEnterpriseR
#  # install.packages("opencgaEnterpriseR_4.0.0.tar.gz", repos=NULL, type="source")

## ----message=FALSE, eval=FALSE, warning=FALSE, include=FALSE------------------
#  library(opencgaEnterpriseR)
#  library(glue)
#  library(dplyr)
#  library(tidyr)
...
#  con <- initopencgaEnterpriseR(opencgaConfig=conf)
...
## ---- eval=FALSE, results='hide'----------------------------------------------
#  projects <- projectClient(opencgaEnterpriseR = con, endpointName = "search")
#  getResponseResults(projects)[[1]][,c("id","name", "description")]

## ---- eval=FALSE, results='hide'----------------------------------------------
#  projects <- projectClient(opencgaEnterpriseR=con, project="population", endpointName="studies")
#  getResponseResults(projects)[[1]][,c("id","name", "description")]
...
#  
#  # Study name
#  study_result <- studyClient(opencgaEnterpriseR = con, studies = study, endpointName = "info")
#  study_name <- getResponseResults(study_result)[[1]][,"name"]
#  # Number of samples in this study
#  count_samples <- sampleClient(opencgaEnterpriseR = con, endpointName = "search",
#                                params=list(study=study, count=TRUE, limit=0))
#  num_samples <- getResponseNumMatches(count_samples)
#  # Number of individuals
#  count_individuals <- individualClient(opencgaEnterpriseR = con, endpointName = "search",
#                                        params=list(study=study, count=TRUE, limit=0))
#  num_individuals <- getResponseNumMatches(count_individuals)
#  # Number of clinical cases
#  count_cases <- clinicalClient(opencgaEnterpriseR = con, endpointName = "search",
#                                params=list(study=study, count=TRUE, limit=0))
#  num_cases <- getResponseNumMatches(count_cases)
#  # Number of variants
#  count_variants <- variantClient(opencgaEnterpriseR = con, endpointName = "query",
#                                  params=list(study=study, count=TRUE, limit=0))
...
#  variant_example <- variantClient(opencgaEnterpriseR = con, endpointName = "query",
...
#  variant_result <- variantClient(opencgaEnterpriseR = con, endpointName = "querySample",
...
#  variant_result <- variantClient(opencgaEnterpriseR = con, endpointName = "query",
...
#  variants_agg <- variantClient(opencgaEnterpriseR = con, endpointName = "aggregationStats",
