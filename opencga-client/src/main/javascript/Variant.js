 ...
 * Autogenerated on: 2022-09-22 17:48:47
 * 
...
export default class Variant extends OpenCGAParentClass {
    ...
    * @param {String} [params.catalogues] - File name containing mutational catalogues. Each sample catalogue is in a column, with sample
    *     names as column headers and channel.
    * @param {String} [params.cataloguesContent] - Mutational catalogues. Each sample catalogue is in a column, with sample names as column
    *     headers and channel.
    * @param {String} [params.fitMethod = "FitMS"] - Either Fit or FitMS. If not specified then FitMS. The default value is FitMS.
    * @param {Number} [params.nBoot] - Number of bootstrap to be used.
    * @param {String} [params.sigVersion = "RefSigv2"] - Either COSMICv2, COSMICv3.2, RefSigv1 or RefSigv2. If not specified RefSigv2. The
    *     default value is RefSigv2.
    * @param {String} [params.organ] - When using RefSigv1 or RefSigv2 as SIGVERSION, organ-specific signatures will be used. If SIGVERSION
    *     is COSMICv2 or COSMICv3.2, then a selection of signatures found in the given organ will be used. Available organs depend on the
    *     selected SIGVERSION. For RefSigv1 or RefSigv2: Biliary, Bladder, Bone_SoftTissue, Breast, Cervix (v1 only), CNS, Colorectal,
    *     Esophagus, Head_neck, Kidney, Liver, Lung, Lymphoid, NET (v2 only), Oral_Oropharyngeal (v2 only), Ovary, Pancreas, Prostate, Skin,
    *     Stomach, Uterus.
    * @param {Number} [params.thresholdPerc = "5f"] - Threshold in percentage of total mutations in a sample, only exposures larger than
    *     THRPERC are considered. If not specified 5. The default value is 5f.
    * @param {Number} [params.thresholdPval = "0.05f"] - P-value to determine the empirical probability that the exposure is lower than the
    *     threshold. If not specified then 0.05. The default value is 0.05f.
    * @param {Number} [params.maxRareSigs = "1"] - Maximum number of rare signatures that are allowed to be present in each sample. If not
    *     specified 1. The default value is 1.
    * @param {String} [params.signaturesFile = "1"] - The file name containing mutational signatures. Each signature is in a column, with
    *     signature names as column hearders and channel names as row names in the first column with no header. Each column must sum to 1. Use
    *     only to provide your own signatures. When fitmethod=FitMS, these signatures are considered common signatures. The default value is 1.
    * @param {String} [params.rareSignaturesFile = "1"] - The file name containing mutational signatures. Each signature is in a column,
    *     with signature names as column hearders and channel names as row names in the first column with no header. Each column must sum to 1.
    *     Use only to provide your own signatures. When fitmethod=FitMS, these signatures are considered rare signatures. The default value is
    *     1.
    * @returns {Promise} Promise object in the form of RestResponse instance.
    */
    ...
    * plot are calculated.
    * @param {Object} data - Sample QC analysis params. Mutational signature and genome plot are calculated for somatic samples only.
    * @param {Object} [params] - The Object containing the following optional parameters:
    * @param {String} [params.study] - Study [[user@]project:]study where study and project can be either the ID or UUID.
    * @param {String} [params.jobId] - Job ID. It must be a unique string within the study. An ID will be autogenerated automatically if not
    *     provided.
