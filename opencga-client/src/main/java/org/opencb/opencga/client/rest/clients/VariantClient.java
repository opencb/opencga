...
public class VariantClient extends AbstractParentClient {
     ...
     *       catalogues: File name containing mutational catalogues. Each sample catalogue is in a column, with sample names as column
     *            headers and channel.
     *       cataloguesContent: Mutational catalogues. Each sample catalogue is in a column, with sample names as column headers and
     *            channel.
     *       fitMethod: Either Fit or FitMS. If not specified then FitMS.
     *       nBoot: Number of bootstrap to be used.
     *       sigVersion: Either COSMICv2, COSMICv3.2, RefSigv1 or RefSigv2. If not specified RefSigv2.
     *       organ: When using RefSigv1 or RefSigv2 as SIGVERSION, organ-specific signatures will be used. If SIGVERSION is COSMICv2 or
     *            COSMICv3.2, then a selection of signatures found in the given organ will be used. Available organs depend on the selected
     *            SIGVERSION. For RefSigv1 or RefSigv2: Biliary, Bladder, Bone_SoftTissue, Breast, Cervix (v1 only), CNS, Colorectal,
     *            Esophagus, Head_neck, Kidney, Liver, Lung, Lymphoid, NET (v2 only), Oral_Oropharyngeal (v2 only), Ovary, Pancreas,
     *            Prostate, Skin, Stomach, Uterus.
     *       thresholdPerc: Threshold in percentage of total mutations in a sample, only exposures larger than THRPERC are considered. If
     *            not specified 5.
     *       thresholdPval: P-value to determine the empirical probability that the exposure is lower than the threshold. If not specified
     *            then 0.05.
     *       maxRareSigs: Maximum number of rare signatures that are allowed to be present in each sample. If not specified 1.
     *       signaturesFile: The file name containing mutational signatures. Each signature is in a column, with signature names as column
     *            hearders and channel names as row names in the first column with no header. Each column must sum to 1. Use only to
     *            provide your own signatures. When fitmethod=FitMS, these signatures are considered common signatures.
     *       rareSignaturesFile: The file name containing mutational signatures. Each signature is in a column, with signature names as
     *            column hearders and channel names as row names in the first column with no header. Each column must sum to 1. Use only to
     *            provide your own signatures. When fitmethod=FitMS, these signatures are considered rare signatures.
     * @return a RestResponse object.
     * @throws ClientException ClientException if there is any server error.
     */
