...
public class AlignmentClient extends AbstractParentClient {
     ...
     * @throws ClientException ClientException if there is any server error.
     */
    public RestResponse<Job> genecoveragestatsQc(AlignmentGeneCoverageStatsParams data, ObjectMap params) throws ClientException {
        params = params != null ? params : new ObjectMap();
        params.put("body", data);
        return execute("analysis", null, "alignment/qc", null, "genecoveragestats", params, POST, Job.class);
    }

    /**
     * Compute quality control (QC) metrics for a given alignment file (including samtools stats, samtools flag stats, FastQC and HS
     *     metrics).
     * @param data Alignment quality control (QC) parameters. The BAM file is mandatory ever but the BED file and the dictionary files are
     *     only mandatory for computing hybrid-selection (HS) metrics.
     * @param params Map containing any of the following optional parameters.
     *       study: study.
     *       jobId: Job ID. It must be a unique string within the study. An ID will be autogenerated automatically if not provided.
     *       jobDependsOn: Comma separated list of existing job IDs the job will depend on.
