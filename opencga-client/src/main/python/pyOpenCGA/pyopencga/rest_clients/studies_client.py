...
class Studies(_ParentRestClient):
    ...
    def update(self, study, data, **options):
        """
    ...
    def update_groups(self, study, data, **options):
        ...
        return self._post('groups/update', query_id=study, data=data, **options)

    def update_users(self, study, group, data, **options):
        """
        ...
        :param str action: Action to be performed: ADD, SET or REMOVE users to/from a group
        :param dict data: JSON containing the parameters
        """
    ...
    def create_groups(self, study, data, **options):
        ...
        PATH: /{apiVersion}/studies/{study}/groups/create

        :param str study: Study [[user@]project:]study where study and project can be either the ID or UUID
        :param dict data: JSON containing the parameters
        """

        return self._post('groups/create', query_id=study, data=data, **options)
    ...
    def update_permission_rules(self, study, entity, data, **options):
        ...
        :param str study: Study [[user@]project:]study where study and project can be either the ID or UUID
        :param str entity: Entity where the permission rules should be applied to
        :param str action: Action to be performed: ADD to add a new permission rule; REMOVE to remove all permissions assigned by an existing permission rule (even if it overlaps any manual permission); REVERT to remove all permissions assigned by an existing permission rule (keep manual overlaps); NONE to remove an existing permission rule without removing any permissions that could have been assigned already by the permission rule.
        :param dict data: JSON containing the permission rule to be created or removed.
        """
        options['entity'] = entity
        return self._post('permissionRules/update', query_id=study, data=data, **options)
    ...
    def update_variable_sets(self, study, data, **options):
        ...
        :param str study: Study [[user@]project:]study where study and project can be either the ID or UUID
        :param str action: Action to be performed: ADD or REMOVE a variableSet
        :param dict data: JSON containing the VariableSet to be created or removed.
        """

        return self._post('variableSets/update', query_id=study, data=data, **options)

    def update_variables(self, study, variable_set, data, **options):
        """
        ...
        return self._post('variableSets', query_id=study, subcategory='variables/update', second_query_id=variable_set, data=data, **options)
