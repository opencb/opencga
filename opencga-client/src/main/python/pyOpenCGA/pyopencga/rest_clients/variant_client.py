...
class Variant(_ParentRestClient):
    ...
    def aggregation_stats(self, **options):
        ...
        :param fields: List of facet fields separated by semicolons, e.g.: studies;type. For nested faceted fields use >>,
        ...
        :param transcriptionFlag: List of transcript annotation flags. e.g. CCDS, basic, cds_end_NF, mRNA_end_NF, cds_start_NF, mRNA_start_NF,
            seleno
    ...
    def sample_data(self, variant, **options):
        """
        Get sample data of a given variant
        URL: /{apiVersion}/analysis/variant/{variant}/sampleData
        ...
        :param merge: Do not group by genotype. Return all genotypes merged (bool: ['true','false' default]).
        :param id: List of IDs, these can be rs IDs (dbSNP) or variants in the format chrom:start:ref:alt, e.g. rs116600158,19:7177679:C:T
        :param limit: Number of results to be returned in the queries
        :param skip: Number of results to skip in the queries
        """

        return self._get('sampleData', query_id=variant, **options)
    ...
    def index(self, file, **options):
        ...
        :param transform: Boolean ['true','false' default] indicating that only the transform step will be run
        :param load: Boolean ['true','false' default] indicating that only the load step will be run
        :param merge: Currently two levels of merge are supported: "basic" mode merge genotypes of the same variants while "advanced" merge
            multiallelic and overlapping variants. Available values ['ADVANCED' as default, 'BASIC']
        :param includeExtraFields: Index including other FORMAT fields. Use "all", "none", or CSV with the fields to load.
        :param aggregated: Default 'none'. Type of aggregated VCF file: none, basic, EVS or ExAC
        :param calculateStats: Calculate indexed variants statistics after the load step (bool: ['true','false' default]).
        :param annotate: Annotate indexed variants after the load step (bool: ['true','false' default]).
        :param overwrite: Overwrite annotations already present in variants (bool: ['true','false' default]).
        :param indexSearch: Add files to the secondary search index (bool: ['true','false' default]).
        :param resume: Resume a previously failed indexation (bool: ['true','false' default]).
        :param loadSplitData: Indicate that the variants from a sample (or group of samples) split into different files (by chromosome,
            by type, ...) (bool: ['true','false' default]).
    ...
    def query(self, **options):
        ...
        :param histogram: Calculate histogram (bool: ['true','false' default]). Requires one region.
        ...
        :param alternate: Main alternate allele
        :param project: Project [user@]project where project can be either the ID or the alias
        :param study: Filter variants from the given studies, these can be either the numeric ID or the alias with the format
            user@project:study
        ...
        :param genotype: Samples with a specific genotype: {samp_1}:{gt_1}(,{gt_n})(;{samp_n}:{gt_1}(,{gt_n}))* e.g. HG0097:0/0;HG0098:0/1,1/1.
            Genotype aliases accepted: HOM_REF, HOM_ALT, HET, HET_REF, HET_ALT and MISS e.g. HG0097:HOM_REF;HG0098:HET_REF,HOM_ALT. This will
        ...
        :param familySegregation: Filter by mode of inheritance from a given family. Accepted values: [ monoallelic, monoallelicIncompletePenetrance,
            biallelic, biallelicIncompletePenetrance, XlinkedBiallelic, XlinkedMonoallelic, Ylinked, MendelianError, DeNovo, CompoundHeterozygous ]
        ...
        :param modeOfInheritance: Filter by mode of inheritance from a given family. Accepted values: [ monoallelic,
    ...
    def variant_annotation_metadata(self, **options):
        """
    ...
    def calculate_family_genotypes(self, family, mode_of_inheritance, **options):
        ...
        :param modeOfInheritance: Mode of inheritance ['MONOALLELIC' default, 'MONOALLELIC_NOT_IMPRINTED', 'MONOALLELIC_MATERNALLY_IMPRINTED',
            'MONOALLELIC_PATERNALLY_IMPRINTED', 'BIALLELIC', 'MONOALLELIC_AND_BIALLELIC', 'MONOALLELIC_AND_MORE_SEVERE_BIALLELIC',
            'XLINKED_BIALLELIC', 'XLINKED_MONOALLELIC', 'YLINKED', 'MITOCHONDRIAL', 'DE_NOVO', 'COMPOUND_HETEROZYGOUS', 'UNKNOWN'].
