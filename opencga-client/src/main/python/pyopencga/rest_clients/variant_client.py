...
class Variant(_ParentRestClient):
    ...
    def run_roh(self, data=None, **options):
        """
        Analysis to calculate and visualise regions of homozygosity (ROHs) for
            a given sample.
        PATH: /{apiVersion}/analysis/variant/roh/run

        :param dict data: ROH parameters. (REQUIRED)
        :param str study: Study [[organization@]project:]study where study and
            project can be either the ID or UUID.
        :param str job_id: Job ID. It must be a unique string within the
            study. An ID will be autogenerated automatically if not provided.
        ...
        return self._post(category='analysis', resource='run', subcategory='variant/roh', data=data, **options)
