...
class Study(_ParentRestClient):
    ...
    def search_audit(self, study, **options):
        ...
        :param str resource: Resource involved.
    ...
    def pipelines(self, study, **options):
        """
        Return the pipelines available for the study.
        PATH: /{apiVersion}/studies/{study}/pipelines

        :param str study: Study [[user@]project:]study where study and project
            can be either the ID or UUID. (REQUIRED)
        :param str pipeline_id: Pipeline id. If provided, it will only fetch
            the requested pipeline.
        """

        return self._get(category='studies', resource='pipelines', query_id=study, **options)

    def update_pipelines(self, study, data=None, **options):
        """
        Add or remove a pipeline.
        PATH: /{apiVersion}/studies/{study}/pipelines/update

        :param dict data: JSON containing the parameters. (REQUIRED)
        :param str study: Study [[user@]project:]study where study and project
            can be either the ID or UUID. (REQUIRED)
        :param str action: Action to be performed: ADD, REMOVE or REPLACE a
            pipeline. Allowed values: ['ADD', 'REMOVE', 'REPLACE']
        """

        return self._post(category='studies', resource='update', query_id=study, subcategory='pipelines', data=data, **options)
