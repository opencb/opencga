...
public class ExecutionInternal extends Internal implements Cloneable {
    ...
    public ExecutionInternal(String registrationDate, Enums.ExecutionStatus status, JobInternalWebhook webhook, List<Event> events) {
    ...
    public static ExecutionInternal init() {
        return new ExecutionInternal(TimeUtils.getTime(), new Enums.ExecutionStatus(), null, new ArrayList<>());
    }
    ...
    public ExecutionInternal clone() throws CloneNotSupportedException {
        return new ExecutionInternal(registrationDate, status, webhook.clone(), new LinkedList<>(events));
    }
