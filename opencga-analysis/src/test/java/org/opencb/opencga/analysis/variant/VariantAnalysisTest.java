...
public class VariantAnalysisTest {
    ...
    public void setUp() throws Throwable {
        if (!indexed) {
            ...
            List<Sample> samples = file.getSamples().subList(0, 2).stream().map(ResourceReference::toSample).collect(Collectors.toList());
            catalogManager.getCohortManager().create(STUDY, "c1", null, null, samples, null, null, token);
            samples = file.getSamples().subList(2, 4).stream().map(ResourceReference::toSample).collect(Collectors.toList());
    ...
    public void testVariantStats() throws Exception {
        ...
        List<String> samples = file.getSamples().stream().map(ResourceReference::getId).collect(Collectors.toList());
    ...
    public void testKnockoutGenes() throws Exception {
        ...
        params.setSample(file.getSamples().stream().map(ResourceReference::getId).collect(Collectors.toList()));
