...
public class VariantFileIndexSameNameTest extends AbstractVariantOperationManagerTest {
    ...
    public void testIndex1() throws Exception {
        ...
        Assert.assertEquals(VariantIndexStatus.READY, catalogManager.getFileManager().get(organizationId, studyId, inputFile1.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
        Assert.assertEquals(VariantIndexStatus.NONE, catalogManager.getFileManager().get(organizationId, studyId, inputFile2.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
    }

    @Test
    public void testIndex2() throws Exception {
        indexFile(inputFile2, new QueryOptions(), outputId);
        Assert.assertEquals(VariantIndexStatus.NONE, catalogManager.getFileManager().get(organizationId, studyId, inputFile1.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
        Assert.assertEquals(VariantIndexStatus.READY, catalogManager.getFileManager().get(organizationId, studyId, inputFile2.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
    }
...
//        thrown.expectCause(isA(StoragePipelineException.class));
    ...
    public void testBySteps1() throws Exception {
        ...
        Assert.assertEquals(VariantIndexStatus.TRANSFORMED, catalogManager.getFileManager().get(organizationId, studyId, inputFile1.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
        Assert.assertEquals(VariantIndexStatus.NONE, catalogManager.getFileManager().get(organizationId, studyId, inputFile2.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
    ...
    public void testBySteps2() throws Exception {
        ...
        Assert.assertEquals(VariantIndexStatus.NONE, catalogManager.getFileManager().get(organizationId, studyId, inputFile1.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
        Assert.assertEquals(VariantIndexStatus.TRANSFORMED, catalogManager.getFileManager().get(organizationId, studyId, inputFile2.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
        loadFile(transformFile, new QueryOptions(), outputId);
        Assert.assertEquals(VariantIndexStatus.NONE, catalogManager.getFileManager().get(organizationId, studyId, inputFile1.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
        Assert.assertEquals(VariantIndexStatus.READY, catalogManager.getFileManager().get(organizationId, studyId, inputFile2.getPath(), null, sessionId).first().getInternal().getVariant().getIndex().getStatus().getId());
    }
