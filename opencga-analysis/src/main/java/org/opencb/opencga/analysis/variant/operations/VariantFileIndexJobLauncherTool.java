        ...
        try (DBIterator<File> dbIterator = getCatalogManager().getFileManager()
            ...
            while (dbIterator.hasNext() && submittedJobs != maxJobs) {
                ...
                if (runningJobs == 0) {
                    ...
                    Execution job = catalogManager.getExecutionManager().submit(getStudy(), VariantIndexOperationTool.ID,
                            Enums.Priority.MEDIUM, indexParams.toParams(new ObjectMap(ParamConstants.STUDY_PARAM, study)), jobId,
                            "Job generated by " + getId(), Collections.emptyList(), Collections.emptyList(), getToken()).first();
