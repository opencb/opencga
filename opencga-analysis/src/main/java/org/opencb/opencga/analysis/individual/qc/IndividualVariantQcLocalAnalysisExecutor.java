...
public class IndividualVariantQcLocalAnalysisExecutor extends IndividualVariantQcAnalysisExecutor implements StorageToolExecutor {

    @Override
    public void run() throws ToolExecutorException {
        Path configPath = getOutDir().resolve(CONFIG_FILENAME);
        ObjectWriter objectWriter = JacksonUtils.getDefaultObjectMapper().writerFor(IndividualQcAnalysisParams.class);
        try {
        ...
        VariantQcAnalysisExecutorUtils.run(INDIVIDUAL_QC_TYPE, getVcfPath(), getJsonPath(), configPath, scriptPath, getOutDir(),
                dockerImage);
    }
}
