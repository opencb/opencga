...
public class NextFlowExecutor extends OpenCgaDockerToolScopeStudy {
    ...
    protected void check() throws Exception {
        ...
        if (externalTool.getWorkflow().getRepository() != null) {
            if (StringUtils.isNotEmpty(externalTool.getWorkflow().getRepository().getId())) {
                dependencyList.add(new ToolDependency(externalTool.getWorkflow().getRepository().getId(), externalTool.getWorkflow().getRepository().getVersion()));
            }
        }
    ...
    protected void run() throws Exception {
        ...
        StringBuilder stringBuilder = new StringBuilder()
                ...
                .append(" run ");
        if (externalTool.getWorkflow().getRepository() != null && StringUtils.isNotEmpty(externalTool.getWorkflow().getRepository().getId())) {
//            stringBuilder.append(workflow.getRepository().getImage()).append(" -with-docker");
            stringBuilder.append(externalTool.getWorkflow().getRepository().getId());
            // Add the repository version if we have it and the user is not specifying it
            if (StringUtils.isNotEmpty(externalTool.getWorkflow().getRepository().getVersion()) && !cliParams.contains("-r ")) {
                stringBuilder
                        .append(" -r ")
                        .append(externalTool.getWorkflow().getRepository().getVersion());
            }
        } else {
            for (WorkflowScript script : externalTool.getWorkflow().getScripts()) {
                if (script.isMain()) {
