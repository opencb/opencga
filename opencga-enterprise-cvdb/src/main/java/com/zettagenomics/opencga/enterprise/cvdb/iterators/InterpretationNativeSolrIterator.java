...
import static com.zettagenomics.opencga.enterprise.cvdb.CvdbSolrEngine.INTERPRETATIONS_COLLECTION_SUFFIX;


public class InterpretationNativeSolrIterator implements Iterator<ClinicalInterpretationSearch>, AutoCloseable {
    ...
    private Iterator<ClinicalInterpretationSearch> solrIterator;
...
//    @Deprecated
//    public InterpretationNativeSolrIterator(Iterator<ClinicalInterpretationSearch> solrIterator) {
//        this.solrIterator = solrIterator;
//    }

    public InterpretationNativeSolrIterator(SolrClient solrClient, SolrQuery solrQuery)
    ...
    public boolean hasNext() {
        ...
        if (solrIterator.hasNext()) {
            ...
            try {
                ...
                solrResponse = solrClient.query(INTERPRETATIONS_COLLECTION_SUFFIX, solrQuery);
                ...
                solrIterator = solrResponse.getBeans(ClinicalInterpretationSearch.class).iterator();
                return solrIterator.hasNext();
            } catch (SolrServerException | IOException e) {
                throw new VariantQueryException("Error searching more interpretations", e);
            }
        }
    }

    @Override
    public ClinicalInterpretationSearch next() {
        // Sanity check
        if (hasNext()) {
