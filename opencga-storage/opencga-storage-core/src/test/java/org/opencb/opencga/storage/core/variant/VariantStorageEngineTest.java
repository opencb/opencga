...
public abstract class VariantStorageEngineTest extends VariantStorageBaseTest {
    ...
    @Test
    public void skipNormalization() throws Exception {
        clearDB(DB_NAME);
        StudyMetadata studyMetadata = newStudyMetadata();
        StoragePipelineResult etlResult = runDefaultETL(smallInputUri, variantStorageEngine, studyMetadata,
                new ObjectMap(VariantStorageOptions.NORMALIZATION_SKIP.key(), true));
        ...
        checkLoadedVariants(variantStorageEngine.getDBAdaptor(), studyMetadata, true, false, true, getExpectedNumLoadedVariants(fileMetadata));
        int indels = 0;
        for (Variant variant : variantStorageEngine.iterable(new Query(VariantQueryParam.TYPE.key(), "indel"), new QueryOptions())) {
            assertNotEquals("", variant.getReference());
            assertNotEquals("", variant.getAlternate());
            indels++;
        }
        assertNotEquals(0, indels);
    }

    @Test
    public void leftAlign() throws Exception {
        clearDB(DB_NAME);
        StudyMetadata studyMetadata = newStudyMetadata();
        getResourceUri("reference/homo_sapiens_grch38_small.fa.gz");
        getResourceUri("reference/homo_sapiens_grch38_small.fa.gz.gzi");
        getResourceUri("reference/homo_sapiens_grch38_small.fa.gz.fai");
        StoragePipelineResult etlResult = runDefaultETL(getResourceUri("reference/unaligned.vcf"), variantStorageEngine, studyMetadata,
                new ObjectMap(VariantStorageOptions.NORMALIZATION_REFERENCE_GENOME.key(), getResourceUri("reference/homo_sapiens_grch38_small.fa.gz").getPath())
        ...
        for (Variant variant : variantStorageEngine.iterable(new VariantQuery().includeSampleAll(), new QueryOptions())) {
            ...
            if (aligned != null) {
                assertEquals(aligned, variant.toString());
            } else {
    ...
    public void loadFromSTDIN() throws Exception {
        ...
        StoragePipelineResult etlResult = runDefaultETL(smallInputUri, variantStorageEngine, null,
        ...
        StudyMetadata studyMetadata = metadataManager.getStudyMetadata(STUDY_NAME);
        VariantFileMetadata fileMetadata = checkTransformedVariants(tempFile.toUri(), studyMetadata);
...
//                        .append("annotation.file.avro", "true")
    ...
    public void multiIndex() throws Exception {
        clearDB(DB_NAME);
        int expectedNumVariants = NUM_VARIANTS;
        ...
        ObjectMap options = new ObjectMap()
                ...
                .append(VariantStorageOptions.STATS_CALCULATE.key(), false)
                .append(VariantStorageOptions.ANNOTATE.key(), false);
        URI file1Uri = getResourceUri("1000g_batches/1-50.filtered.10k.chr22.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes.vcf.gz");
        ...
        assertEquals(50, defaultCohort.getSamples().size());
        ...
        assertEquals(100, defaultCohort.getSamples().size());
        ...
        URI file3Uri = getResourceUri("1000g_batches/101-150.filtered.10k.chr22.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes.vcf.gz");
        runDefaultETL(file3Uri, variantStorageEngine, studyMetadataMultiFile, options);
        defaultCohort = metadataManager.getCohortMetadata(studyMetadataMultiFile.getId(), StudyEntry.DEFAULT_COHORT);
        assertEquals(150, defaultCohort.getSamples().size());
        assertFalse(defaultCohort.isStatsReady());
        ...
        options.append(VariantStorageOptions.STATS_CALCULATE.key(), true);
        URI file4Uri = getResourceUri("1000g_batches/151-200.filtered.10k.chr22.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes.vcf.gz");
        runDefaultETL(file4Uri, variantStorageEngine, studyMetadataMultiFile, options);
        defaultCohort = metadataManager.getCohortMetadata(studyMetadataMultiFile.getId(), StudyEntry.DEFAULT_COHORT);
        assertEquals(200, defaultCohort.getSamples().size());
        ...
        URI file5Uri = getResourceUri("1000g_batches/201-250.filtered.10k.chr22.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes.vcf.gz");
        ...
        defaultCohort = metadataManager.getCohortMetadata(studyMetadataMultiFile.getId(), StudyEntry.DEFAULT_COHORT);
        assertEquals(250, defaultCohort.getSamples().size());
        assertTrue(metadataManager.getIndexedFiles(studyMetadataMultiFile.getId()).contains(fileId1));
        assertTrue(metadataManager.getIndexedFiles(studyMetadataMultiFile.getId()).contains(fileId2));
        assertTrue(metadataManager.getIndexedFiles(studyMetadataMultiFile.getId()).contains(fileId3));
        assertTrue(metadataManager.getIndexedFiles(studyMetadataMultiFile.getId()).contains(fileId4));
...
//                studyMetadata, options.append(VariantStorageEngine.Options.FILE_ID.key(), 6));
    ...
    public void multiRegionIndex() throws Exception {
        ...
        assertEquals(50, cohort.getSamples().size());
...
//        }
    ...
    public void multiRegionIndexFail() throws Exception {
        ...
        StorageEngineException exception = StorageEngineException.alreadyLoadedSamples(
                ...
                IntStream.range(0, 50).mapToObj(i -> "NA128" + i).collect(Collectors.toList()));
        ...
        runDefaultETL(getResourceUri("10k.chr22.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes.vcf.gz"), variantStorageEngine,
                studyMetadata, options.append(VariantStorageOptions.LOAD_SPLIT_DATA.key(), null));
    }
    ...
    public void singleThreadIndex() throws Exception {
        ...
        params.put(VariantStorageOptions.LOAD_ARCHIVE.key(), "false");
        params.put(VariantStorageOptions.LOAD_SAMPLE_INDEX.key(), "false");
        params.put(VariantStorageOptions.LOAD_HOM_REF.key(), true);
...
//        params.put(VariantStorageEngine.Options.INCLUDE_SRC.key(), false);
    ...
    public void indexWithOtherFields(String extraFields) throws Exception {
        ...
        for (Variant variant : reader.read(999)) {
            ...
            Variant loadedVariant = dbAdaptor.get(new VariantQuery().id(variant.toString())
                    .includeSampleData("GT,GL,DS").includeSampleAll(), new QueryOptions()).first();
    ...
    private void checkLoadedVariants(VariantDBAdaptor dbAdaptor, StudyMetadata studyMetadata,
        ...
        DataResult<Long> count = dbAdaptor.count();
        if (expectedNumVariants >= 0) {
            assertEquals(expectedNumVariants, count.getNumMatches());
        }
...
//        }
        ...
        for (Variant variant : dbAdaptor.iterable(new VariantQuery().includeSampleAll().sampleLimit(3000), new QueryOptions())) {
                    ...
                    try {
                        ...
                        assertEquals(variant + " has incorrect stats for cohort \"" + cohort.getName() + "\":" + cohort.getId(),
                                ...
                                variantStats.getGenotypeCount().values()
                                        ...
                                        .sum() + variantStats.getMissingGenotypeCount());
                        assertEquals(variant + " has incorrect stats for cohort \"" + cohort.getName() + "\":" + cohort.getId(),
                                cohort.getSamples().size(),
                                variantStats.getSampleCount() + variantStats.getMissingGenotypeCount());
