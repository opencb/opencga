...
import org.opencb.opencga.storage.core.variant.index.core.filters.RangeIndexFieldFilter;
...
import static org.opencb.opencga.storage.core.variant.index.sample.annotation.SampleIndexVariantAnnotationConverterTest.*;
...
//            value.setVariants(new SampleIndexVariantBiConverter().toVariantsCountIterator(value.getCount()));
    ...
    private SampleIndexEntry getSampleIndexEntry1() {
        ...
        //{0.001, 0.005, 0.01};
        SampleIndexEntry.SampleIndexGtEntry gtEntry = new SampleIndexVariantAnnotationBuilder(schema)
...
//                .collect(Collectors.toMap(cell -> Bytes.toString(CellUtil.cloneQualifier(cell)), CellUtil::cloneValue)).get("_PF_0/1");
        ...
        entry.getGtEntry("0/1")
                .setAnnotationIndex(gtEntry.getAnnotationIndex())
                .setCtBtTfIndex(gtEntry.getCtBtTfIndex())
                .setConsequenceTypeIndex(gtEntry.getConsequenceTypeIndex())
                .setBiotypeIndex(gtEntry.getBiotypeIndex())
                .setPopulationFrequencyIndex(gtEntry.getPopulationFrequencyIndex())
                .setCount(5)
                .setVariants(toBytes(
                        "1:10:A:T",
                        "1:20:A:T",
    ...
    private SampleIndexEntry getSampleIndexEntry2() {
        ...
        entry.getGtEntry("0/1")
                ...
                .setConsequenceTypeIndex(gtEntry.getConsequenceTypeIndex())
                .setBiotypeIndex(gtEntry.getBiotypeIndex())
                .setTranscriptFlagIndex(gtEntry.getTranscriptFlagIndex())
                .setPopulationFrequencyIndex(gtEntry.getPopulationFrequencyIndex())
                .setCount(5)
                .setVariants(toBytes(
                        "1:10:A:T",
...
//    }
    ...
    private SingleSampleIndexQuery getSingleSampleIndexQuery(SampleAnnotationIndexQuery annotationIndexQuery, Map<String, Values<SampleFileIndexQuery>> fileFilterMap) {
        return new SampleIndexQuery(
                schema, Collections.emptyList(), 0, null, "study", Collections.singletonMap("S1", Arrays.asList("0/1", "1/1")), Collections.emptySet(), null, Collections.emptyMap(), Collections.emptyMap(), fileFilterMap, annotationIndexQuery, Collections.emptySet(), null, false, VariantQueryUtils.QueryOperation.AND, null)
                .forSample("S1");
    }
}