...
//        checkIterator(numVariants, variants, () -> toSampleIndexEntry(batchStart, bytesOldOffset, 10, bytesOld.length));
        ...
        checkIterator(numVariants, dummyVariants, () -> {
            ...
            gtEntry.setCount(variants.size());
            return entry;
        }, true);

    }

    private SampleIndexEntry toSampleIndexEntry(int batchStart, byte[] bytes, int offset, int length) {
        SampleIndexEntry entry = new SampleIndexEntry(0, "1", batchStart);
        SampleIndexEntry.SampleIndexGtEntry gtEntry = entry.getGtEntry("0/1");
    ...
    private void checkIterator(int numVariants, List<Variant> variants, Supplier<SampleIndexEntry> factory) {
        checkIterator(numVariants, variants, factory, false);
    }

    private void checkIterator(int numVariants, List<Variant> variants, Supplier<SampleIndexEntry> factory, boolean onlyCount) {
    ...
    private void checkIterator(int numVariants, List<Variant> variants, SampleIndexEntry entry, boolean annotated, boolean onlyCount) {
        SampleIndexEntry.SampleIndexGtEntry gtEntry = entry.getGtEntry("0/1");
        if (annotated) {
            ...
            gtEntry.setAnnotationIndex(annot);
        }
        SampleIndexEntryIterator iterator = converter.toVariantsIterator(entry.getChromosome(), entry.getBatchStart(), gtEntry, onlyCount);
        int i = 0;
