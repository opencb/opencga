...
public class VariantManagerFetchTest extends AbstractVariantStorageOperationTest {
    ...
    public void testQueryExcludeSamples() throws Exception {
        ...
        // Without studies
        Map<String, List<Sample>> longListMap = variantManager.checkSamplesPermissions(query, queryOptions, mockVariantDBAdaptor().getVariantStorageMetadataManager(), sessionId);
        ...
        longListMap = variantManager.checkSamplesPermissions(query, queryOptions, mockVariantDBAdaptor().getVariantStorageMetadataManager(), sessionId);
        Assert.assertEquals(Collections.singletonMap(studyFqn, Collections.emptyList()), longListMap);
    }
