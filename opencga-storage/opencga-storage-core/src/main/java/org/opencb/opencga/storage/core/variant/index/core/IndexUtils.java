...
public final class IndexUtils {
    ...
    public static byte[] countPerBitToBytes(int[] counts) {
        ...
        for (int count : counts) {
            offset += encodePUnsignedInt(count, bytes, offset);
        }
        return bytes;
    }
    ...
    public static int decodePUnsignedInt(byte[] bytes, int offset) {
        return Bytes.toInt(bytes, offset);
    }

    public static int encodePUnsignedInt(int value, byte[] bytes, int offset) {
        Bytes.putInt(bytes, offset, value);
        return Bytes.SIZEOF_INT;
    }

    public static int decodePInt(byte[] bytes, int offset) {
        return ((bytes[offset + 0] & 0xFF) ^ 0x80) << 24 // Flip sign bit back
                | (bytes[offset + 1] & 0xFF) << 16
                | (bytes[offset + 2] & 0xFF) << 8
                | (bytes[offset + 3] & 0xFF);
    }

    public static int encodePInt(int value, byte[] bytes, int offset) {
        bytes[offset + 0] = (byte) ((value >> 24) ^ 0x80); // Flip sign bit so that INTEGER is binary comparable
    ...
    public static String decodePVarchar(byte[] bytes, int offset, int length) {
        if (length <= 0) {
            return null;
        }
    ...
    public static int encodePVarchar(String value, byte[] bytes, int offset) {
        if (value == null) {
            return 0;
        }
