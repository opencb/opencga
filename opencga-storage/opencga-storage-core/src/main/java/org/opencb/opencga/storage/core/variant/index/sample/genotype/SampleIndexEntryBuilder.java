...
public class SampleIndexEntryBuilder {
    ...
    /**
     * Create a SampleIndexEntryBuilder for one specific Sample and chunk region (chr+pos).
     *
    ...
    public SampleIndexEntry buildEntry() {
        ...
        for (SampleIndexGtEntryBuilder gtBuilder : gts.values()) {
            entry.getGts().put(gtBuilder.getGt(), gtBuilder.build());
        }
        ...
        entry.setDiscrepancies(discrepancies);
...
//    }
    ...
    protected abstract class SampleIndexGtEntryBuilder {
        ...
        public SampleIndexEntry.SampleIndexGtEntry build() {
            ...
            List<Variant> variants = new ArrayList<>(variantEntries.size());
            for (SampleIndexVariant gtEntry : variantEntries) {
            ...
            entry.setVariants(variantsBytes);
            entry.setCount(variants.size());
            entry.setFileIndex(fileIndexBuffer.getBuffer());
            int position = fileDataIndexBuffer.position();
            if (position > 0) {
                fileDataIndexBuffer.rewind();
                fileDataIndexBuffer.limit(position);
                entry.setFileDataIndex(fileDataIndexBuffer.array(), 0, position);
            }
    ...
    private class SampleIndexGtEntryBuilderWithPartialBuilds extends SampleIndexGtEntryBuilderAssumeOrdered {
        ...
        public SampleIndexEntry.SampleIndexGtEntry build() {
            ...
            // Add to put
            SampleIndexEntry.SampleIndexGtEntry entry = new SampleIndexEntry.SampleIndexGtEntry(gt);
            entry.setVariants(variantsBuffer);
            entry.setCount(variantsCount);
            entry.setFileIndex(this.fileIndexBuffer.toByteArray());
            if (fileDataBuffer.size() > 0) {
                entry.setFileDataIndex(fileDataBuffer.getBuffer(), 0, fileDataBuffer.size());
            }
            return entry;
        }
    }
