...
public final class SampleIndexSchema {
    ...
    public static final String MENDELIAN_ERROR_COLUMN = "ME";
    public static final byte[] MENDELIAN_ERROR_COLUMN_BYTES = Bytes.toBytes(MENDELIAN_ERROR_COLUMN);
    public static final char META_PREFIX = '_';
    public static final byte[] META_PREFIX_BYTES = Bytes.toBytes("_");
    public static final String PARENTS_PREFIX = META_PREFIX + "P_";
    public static final byte[] PARENTS_PREFIX_BYTES = Bytes.toBytes(PARENTS_PREFIX);
    public static final String FILE_PREFIX = META_PREFIX + "F_";
    public static final byte[] FILE_PREFIX_BYTES = Bytes.toBytes(FILE_PREFIX);
    public static final String FILE_DATA_PREFIX = META_PREFIX + "FD_";
    public static final byte[] FILE_DATA_PREFIX_BYTES = Bytes.toBytes(FILE_DATA_PREFIX);
    public static final String GENOTYPE_COUNT_PREFIX = META_PREFIX + "C_";
    public static final byte[] GENOTYPE_COUNT_PREFIX_BYTES = Bytes.toBytes(GENOTYPE_COUNT_PREFIX);
    public static final String GENOTYPE_DISCREPANCY_COUNT = META_PREFIX + "DC";
    public static final byte[] GENOTYPE_DISCREPANCY_COUNT_BYTES = Bytes.toBytes(GENOTYPE_DISCREPANCY_COUNT);

    public static final String ANNOTATION_SUMMARY_PREFIX = META_PREFIX + "A_";
    public static final byte[] ANNOTATION_SUMMARY_PREFIX_BYTES = Bytes.toBytes(ANNOTATION_SUMMARY_PREFIX);
    public static final String ANNOTATION_SUMMARY_COUNT_PREFIX = META_PREFIX + "AC_";
    public static final byte[] ANNOTATION_SUMMARY_COUNT_PREFIX_BYTES = Bytes.toBytes(ANNOTATION_SUMMARY_COUNT_PREFIX);

    // Consequence Type
    public static final String ANNOTATION_CT_PREFIX = META_PREFIX + "CT_";
    public static final byte[] ANNOTATION_CT_PREFIX_BYTES = Bytes.toBytes(ANNOTATION_CT_PREFIX);
    // Biotype
    public static final String ANNOTATION_BT_PREFIX = META_PREFIX + "BT_";
    public static final byte[] ANNOTATION_BT_PREFIX_BYTES = Bytes.toBytes(ANNOTATION_BT_PREFIX);
    // Transcript Flag
    public static final String ANNOTATION_TF_PREFIX = META_PREFIX + "TF_";
    public static final byte[] ANNOTATION_TF_PREFIX_BYTES = Bytes.toBytes(ANNOTATION_TF_PREFIX);
    // ConsequenceType + Biotype + TranscriptFlag combination combination
    public static final String ANNOTATION_CT_BT_TF_PREFIX = META_PREFIX + "CBT_";
    public static final byte[] ANNOTATION_CT_BT_TF_PREFIX_BYTES = Bytes.toBytes(ANNOTATION_CT_BT_TF_PREFIX);
    // PopFreq
    public static final String ANNOTATION_POP_FREQ_PREFIX = META_PREFIX + "PF_";
    public static final byte[] ANNOTATION_POP_FREQ_PREFIX_BYTES = Bytes.toBytes(ANNOTATION_POP_FREQ_PREFIX);
    // Clinical
    public static final String ANNOTATION_CLINICAL_PREFIX = META_PREFIX + "CL_";
    public static final byte[] ANNOTATION_CLINICAL_PREFIX_BYTES = Bytes.toBytes(ANNOTATION_CLINICAL_PREFIX);
...
//    }
    ...
    public static int getExpectedSize(String chromosome) {
        ...
        if (chromosome == null) {
            ...
            expectedSize = Integer.BYTES + chromosome.length() + 1 + Integer.BYTES;
        }
        return expectedSize;
    }
    ...
    private static int toRowKey(int sample, String chromosome, int position, byte[] rk) {
        int offset = 0;
        offset += IndexUtils.encodePInt(sample, rk, offset);


        if (chromosome != null) {
            offset += IndexUtils.encodePVarchar(chromosome, rk, offset);
            rk[offset] = 0;
            offset++;
            offset += IndexUtils.encodePInt(position / BATCH_SIZE, rk, offset);
        }
        return offset;
    }
    ...
    public static String rowKeyToString(byte[] row) {
        ...
        Object sampleId = IndexUtils.decodePInt(row, 0);
        if (row.length > 5) {
    ...
    public static int sampleIdFromRowKey(byte[] row) {
        return IndexUtils.decodePInt(row, 0);
    }

    public static String chromosomeFromRowKey(byte[] row) {
        return IndexUtils.decodePVarchar(row, 4, row.length - 4 - 1 - 4);
    }

    public static int batchStartFromRowKey(byte[] row) {
        return IndexUtils.decodePInt(row, row.length - 4) * BATCH_SIZE;
    }

    public static boolean isGenotypeColumn(byte[] column, int offset, int length) {
        byte b = column[offset];
        return b != META_PREFIX
                && !Bytes.equals(column, offset, length, MENDELIAN_ERROR_COLUMN_BYTES, 0, MENDELIAN_ERROR_COLUMN_BYTES.length);
    }
    ...
    public static String getGt(byte[] column, int offset, int length, byte[] prefix) {
