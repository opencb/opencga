...
import static org.opencb.opencga.storage.core.variant.index.core.IndexUtils.testIndex;
import static org.opencb.opencga.storage.core.variant.index.core.IndexUtils.testParentsGenotypeCode;
...
public abstract class AbstractSampleIndexEntryFilter<T> {
    ...
    private Collection<T> filter(SampleIndexEntry entry, boolean count) {
        ...
        for (SampleIndexGtEntry gtEntry : gts.values()) {
            ...
            SampleIndexEntryIterator variantIterator = converter
                    .toVariantsIterator(entry.getChromosome(), entry.getBatchStart(), gtEntry, countIterator);
