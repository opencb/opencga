...
public class SampleIndexVariantBiConverter {
    ...
    public List<Variant> toVariants(String chromosome, int batchStart, byte[] bytes, int offset, int length) {
        ...
        SampleIndexEntryIterator it = toVariantsIterator(chromosome, batchStart, gtEntry);
    ...
    public SampleIndexEntryIterator toVariantsIterator(String chromosome, int batchStart, SampleIndexEntry.SampleIndexGtEntry entry,
                                                       boolean onlyCount) {
        if (onlyCount) {
            return toVariantsCountIterator(entry);
        } else {
            return toVariantsIterator(chromosome, batchStart, entry);
        }
    }

    public SampleIndexEntryIterator toVariantsIterator(SampleIndexEntry entry, String gt) {
        return toVariantsIterator(entry.getChromosome(), entry.getBatchStart(), entry.getGts().get(gt));
    }

    public SampleIndexEntryIterator toVariantsIterator(String chromosome, int batchStart, SampleIndexEntry.SampleIndexGtEntry gtEntry) {
...
//            configuration.getFileIndex().getMultiFileIndex().readAndDecode()
    ...
    private Variant toVariantEncodedAlleles(String chromosome, int batchStart, byte[] bytes, int offset) {
        ...
        return VariantKeyFactory.buildVariant(chromosome, start, refAlt[0], refAlt[1], null, null);
    }
    ...
    private Variant toVariant(String chromosome, int batchStart, byte[] bytes, int offset, int referenceLength, int alternateLength) {
        ...
        return VariantKeyFactory.buildVariant(chromosome, start, reference, alternate, null, null);
    }
    ...
    protected String getAlternate(Variant v) {
        return VariantKeyFactory.buildSymbolicAlternate(v);
    }
    ...
    protected int appendString(String str, byte[] bytes, int offset) {
        return IndexUtils.encodePVarchar(str, bytes, offset);
    }

    protected String readString(byte[] bytes, int offset, int length) {
        return IndexUtils.decodePVarchar(bytes, offset, length);
    }
