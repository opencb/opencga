...
public class HBaseToSampleIndexConverter implements Converter<Result, Collection<Variant>> {
    ...
    private static final String PARENTS_PREFIX = META_PREFIX + "P_";
        ...
        if (cell != null && cell.getValueLength() != 0) {
            for (String s : splitValue(cell)) {
                ...
                if (genotypes.isEmpty() || genotypes.contains(gt)) {
                    ...
                    Variant variant = filterAndConvert(variantStr, variantIdx, fileIndexGt, annotationIndexGt, null);
                    if (variant != null) {
        ...
        for (Cell cell : result.rawCells()) {
            if (cell.getQualifierArray()[cell.getQualifierOffset()] != META_PREFIX) {
                ...
                for (String variantStr : splitValue(cell)) {
                    Variant variant = filterAndConvert(variantStr, idx, fileIndexGt, annotationIndexGt, parentsGt);
                    if (variant != null) {
                        variants.add(variant);
                    }
                    idx++;
    ...
    private Variant filterAndConvert(String variant, int idx, byte[] fileIndexGt, byte[] annotationIndexGt, byte[] parentsGt) {
