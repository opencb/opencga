...
class ClinicalAnalysis(_ParentRestClient):
    ...
    def run_pipeline_execute(self, data=None, **options):
        """
        Execute the clinical pipeline that performs QC (e.g.: FastQC), mapping
            (e.g.: BWA), variant calling (e.g., GATK) and variant indexing in
            OpenCGA storage.
        PATH: /{apiVersion}/analysis/clinical/pipeline/execute/run

        :param dict data: Parameters to execute the clinical pipeline.
            (REQUIRED)
        :param str study: study.
        :param str job_id: Job ID. It must be a unique string within the
            study. An ID will be autogenerated automatically if not provided.
        ...
        return self._post(category='analysis', resource='run', subcategory='clinical/pipeline/execute', data=data, **options)

    def run_pipeline_prepare(self, data=None, **options):
        """
        Prepare the clinical pipeline that performs QC (e.g.: FastQC), mapping
            (e.g.: BWA), variant calling (e.g., GATK) and variant indexing in
            OpenCGA storage.
        PATH: /{apiVersion}/analysis/clinical/pipeline/prepare/run

        :param dict data: Parameters to prepare the clinical pipeline.
            (REQUIRED)
        :param str study: study.
        :param str job_id: Job ID. It must be a unique string within the
            study. An ID will be autogenerated automatically if not provided.
