...
public class FileWSServer extends OpenCGAWSServer {
    ...
    public Response downloadAndRegister(
        ...
        return submitJob(studyStr, JobType.NATIVE_TOOL, FetchAndRegisterTask.ID, fetchParams, jobId, jobDescription, dependsOn, jobTags, scheduledStartTime, jobPriority, dryRun);
    }
    ...
    public Response list(@ApiParam(value = ParamConstants.FILE_FOLDER_DESCRIPTION) @PathParam(ParamConstants.FILE_FOLDER) String folder,
        ...
        try {
            ...
            DataResult<File> result = catalogManager.getFileManager().getFilesFromFolder(studyStr, folder, queryOptions, token);
...
//    }
    ...
    @POST
    @Path("/{file}/move")
    @ApiOperation(value = "Move file to a different path", response = File.class)
    @ApiImplicitParams({
            @ApiImplicitParam(name = QueryOptions.INCLUDE, value = ParamConstants.INCLUDE_DESCRIPTION,
                    dataType = "string", paramType = "query"),
            @ApiImplicitParam(name = QueryOptions.EXCLUDE, value = ParamConstants.EXCLUDE_DESCRIPTION,
    ...
    public Response move(
            @ApiParam(value = "File id, UUID or name.") @PathParam(value = "file") String fileIdStr,
            @ApiParam(value = ParamConstants.STUDY_DESCRIPTION) @QueryParam(ParamConstants.STUDY_PARAM) String studyStr,
            @ApiParam(name = "body", value = "Parameters to modify", required = true) FileMoveParams moveParams) {
        try {
            DataResult<File> queryResult = fileManager.move(studyStr, fileIdStr, moveParams.getPath(), queryOptions, token);
            return createOkResponse(queryResult);
        } catch (Exception e) {
            return createErrorResponse(e);
        }
...
//    }
    ...
    public Response postlink(
        ...
        return submitJob(studyStr, JobType.NATIVE_TOOL, PostLinkSampleAssociation.ID, params, jobId, jobDescription, dependsOn, jobTags, scheduledStartTime, jobPriority, dryRun);
    }
    ...
    public Response linkAsync(
        ...
        return submitJob(studyStr, JobType.NATIVE_TOOL, FileLinkTask.ID, params, jobId, jobDescription, dependsOn, jobTags, scheduledStartTime, jobPriority, dryRun);
    }
    ...
    public Response unlink(
        ...
        try {
            ...
            OpenCGAResult<Job> result = catalogManager.getJobManager().submit(studyStr, JobType.NATIVE_TOOL, "files-unlink", Enums.Priority.MEDIUM, params,
                    token);
            return createOkResponse(result);
        } catch (Exception e) {
            return createErrorResponse(e);
    ...
    public Response refresh(
        ...
        try {
            ...
            if (file.getType() == File.Type.FILE) {
                ...
                List<File> result = catalogManager.getFileManager().getFilesFromFolder(studyStr, fileIdStr, null, token).getResults();
...
//    }
    ...
    public Response delete(
        ...
        try {
            ...
            OpenCGAResult<Job> result = catalogManager.getJobManager().submit(studyStr, JobType.NATIVE_TOOL, FileDeleteTask.ID, Enums.Priority.MEDIUM, params,
                    token);
            return createOkResponse(result);
        } catch (Exception e) {
            return createErrorResponse(e);
    ...
    @ApiOperation(value = "Return the acl defined for the file or folder. If member is provided, it will only return the acl for the " +
            "member.", response = FileAclEntryList.class)
    ...
    @ApiOperation(value = "Update the set of permissions granted for the member", response = FileAclEntryList.class)
