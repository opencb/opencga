...
//    private static String ANALYSIS_FOLDER = "analysis";
    ...
    public FileIOManager(String rootdir) throws IOException, IOManagementException {
...
//        tmp = accountProperties.getProperty("OPENCGA.TMP.PATH");
    ...
    public Path getBucketsPath(String accountId) {
        return getUserPath(accountId).resolve(Paths.get(FileIOManager.BUCKETS_FOLDER));
    }

    public Path getBucketPath(String accountId, String bucketId) {
        if (bucketId != null) {
            return getUserPath(accountId).resolve(Paths.get(FileIOManager.BUCKETS_FOLDER, bucketId.toLowerCase()));
        }
        return getUserPath(accountId).resolve(FileIOManager.BUCKETS_FOLDER);
    }

    public Path getProjectPath(String accountId, String projectId) {
        if (projectId != null) {
            return getUserPath(accountId).resolve(Paths.get(FileIOManager.OPENCGA_USER_FOLDER, projectId.toLowerCase()));
        }
        return getUserPath(accountId).resolve(FileIOManager.OPENCGA_USER_FOLDER);
    }
...
//        System.out.println(accountId + " createAccount(): Creating account folder: " + accountHomePath);
        if (Files.exists(accountPath) && Files.isDirectory(accountPath) && Files.isWritable(accountPath)) {
            ...
            try {
                ...
                Files.createDirectory(Paths.get(accountHomePath, accountId, FileIOManager.OPENCGA_USER_FOLDER));
...
//            System.out.println("TEST ERROR The account folder has not been created");
    ...
    public URI createProject(String accountId, String projectId) throws IOManagementException {
        Path folder = Paths.get(accountHomePath, accountId, FileIOManager.OPENCGA_USER_FOLDER, projectId);
        if (Files.exists(folder.getParent()) && Files.isDirectory(folder.getParent())
                && Files.isWritable(folder.getParent())) {
            try {
                folder = Files.createDirectory(folder);
    ...
    public void renameProject(String accountId, String oldProjectId, String newProjectId) throws IOManagementException {
        Path oldFolder = Paths.get(accountHomePath, accountId, FileIOManager.OPENCGA_USER_FOLDER, oldProjectId);
        Path newFolder = Paths.get(accountHomePath, accountId, FileIOManager.OPENCGA_USER_FOLDER, newProjectId);
        try {
            Files.move(oldFolder, newFolder);
        } catch (IOException e) {
