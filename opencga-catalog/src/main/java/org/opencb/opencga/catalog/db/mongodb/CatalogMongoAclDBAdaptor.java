...
import static org.opencb.opencga.catalog.db.mongodb.CatalogMongoDBAdaptor.PRIVATE_STUDY_ID;

/**
 * Created by pfurio on 29/07/16.
 */
public class CatalogMongoAclDBAdaptor<T extends AbstractAclEntry> implements AclDBAdaptor<T> {

    private MongoDBCollection collection;
    private GenericConverter<? extends AbstractAcl, Document> converter;
    private Logger logger;

    public CatalogMongoAclDBAdaptor(MongoDBCollection collection, GenericConverter<? extends AbstractAcl, Document> converter,
                                    Logger logger) {
    ...
    public T createAcl(long resourceId, T acl) throws CatalogDBException {
        ...
        Document update = new Document("$push", new Document(QueryParams.ACL.key(),
                CatalogMongoDBUtils.getMongoDBDocument(acl, "ACL")));
