...
public class ExecutionDaemon extends MonitorParentDaemon {
    ...
    private boolean canBeQueued(String command, String subcommand, int maxJobs) {
        Query query = new Query()
                .append(JobDBAdaptor.QueryParams.STATUS_NAME.key(), Enums.ExecutionStatus.QUEUED + "," + Enums.ExecutionStatus.RUNNING)
                .append(JobDBAdaptor.QueryParams.ATTRIBUTES.key() + "." + Job.OPENCGA_COMMAND, command)
                .append(JobDBAdaptor.QueryParams.ATTRIBUTES.key() + "." + Job.OPENCGA_SUBCOMMAND, subcommand);
        String key = command + " " + subcommand;
        long currentJobs = jobsCountByType.computeIfAbsent(key, k -> {
            try {
                return catalogManager.getJobManager().count(query, token).getNumMatches();
            } catch (CatalogException e) {
                logger.error("Error counting the current number of running and queued \"" + command + " " + subcommand + "\" jobs", e);
                return 0L;
            }
        });
        if (currentJobs >= maxJobs) {
            long now = System.currentTimeMillis();
            Long lastTimeLog = retainedLogsTime.getOrDefault(key, 0L);
            if (now - lastTimeLog > 60000) {
                logger.info("There are {} " + command + " " + subcommand + " jobs running or queued already. "
                        + "Current limit is {}. "
                        + "Halt new " + command + " " + subcommand + " jobs.", currentJobs, maxJobs);
                retainedLogsTime.put(key, now);
            }
            return false;
        } else {
            jobsCountByType.remove(key);
            retainedLogsTime.put(key, 0L);
            return true;
        }
    }
