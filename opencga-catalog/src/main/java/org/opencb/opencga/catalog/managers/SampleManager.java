    ...
    public OpenCGAResult<Sample> search(String studyId, Query query, QueryOptions options, String token) throws CatalogException {
        ...
        try {
            ...
            if (options.getInt(QueryOptions.LIMIT, DEFAULT_LIMIT) > 0) {
                queryResult = sampleDBAdaptor.get(study.getUid(), query, options, userId);
            }
            ...
            try {
                ...
                switch (sampleAclParams.getAction()) {
                    ...
                    case ADD:
                        ...
                        break;
                    case REMOVE:
                        queryResults = authorizationManager.removeAcls(sampleUids, individualUids, members, permissions,
                                Enums.Resource.SAMPLE, resource2);
                        break;
                    case RESET:
                        queryResults = authorizationManager.removeAcls(sampleUids, individualUids, members, null, Enums.Resource.SAMPLE,
                                resource2);
                        break;
                    default:
