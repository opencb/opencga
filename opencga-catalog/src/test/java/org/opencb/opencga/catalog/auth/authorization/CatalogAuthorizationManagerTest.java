...
public class CatalogAuthorizationManagerTest extends GenericTest {
    ...
    public void before() throws Exception {
        ...
        catalogManager.getUserManager().create(studyAdminUser1, studyAdminUser1, "email@ccc.ccc", password, "ASDF", null, Account.FULL, null, null);
        catalogManager.getUserManager().create(studyAdminUser2, studyAdminUser2, "email@ccc.ccc", password, "ASDF", null, Account.FULL, null, null);
        catalogManager.getUserManager().create(memberUser, memberUser, "email@ccc.ccc", password, "ASDF", null, Account.FULL, null, null);
        catalogManager.getUserManager().create(externalUser, externalUser, "email@ccc.ccc", password, "ASDF", null, Account.FULL, null, null);
        ...
        s1 = catalogManager.getStudyManager().create(String.valueOf(p1), "s1", "s1", Study.Type.CASE_CONTROL, null, null, null, null,
                null, null, null, null, null, null, ownerSessionId).first();
        data_d1 = catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/d1/").toString(), null, true, null,
                QueryOptions.empty(), ownerSessionId).first().getUid();
        data_d1_d2 = catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/d1/d2/").toString(), null, false,
                null, QueryOptions.empty(), ownerSessionId).first().getUid();
        data_d1_d2_d3 = catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/d1/d2/d3/").toString(), null,
        false, null, QueryOptions.empty(), ownerSessionId).first().getUid();
        data_d1_d2_d3_d4 = catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/d1/d2/d3/d4/").toString(),
                null, false, null, QueryOptions.empty(), ownerSessionId).first().getUid();
        ...
        catalogManager.getStudyManager().updateGroup(s1.getId(), groupAdmin, new GroupParams(studyAdminUser1 + "," +
...
//        catalogManager.getStudyManager().updateAcl(s1.getId(), groupAdmin, aclParams2, ownerSessionId).get(0);

        Study.StudyAclParams aclParams1 = new Study.StudyAclParams("", AclParams.Action.ADD, AuthorizationManager.ROLE_ANALYST);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), memberUser, aclParams1, studyAdmin1SessionId).get(0);
        Study.StudyAclParams aclParams = new Study.StudyAclParams("", AclParams.Action.ADD, AuthorizationManager.ROLE_LOCKED);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), externalUser, aclParams, studyAdmin1SessionId).get(0);

        fileManager.updateAcl(s1.getId(), Arrays.asList(Long.toString(data_d1)), externalUser, new File.FileAclParams(ALL_FILE_PERMISSIONS,
                AclParams.Action.SET, null), ownerSessionId);
        //        fileManager.updateAcl(s1.getId(), Long.toString(data_d1), groupAdmin, new File.FileAclParams(ALL_FILE_PERMISSIONS,
//                AclParams.Action.SET, null), ownerSessionId);
        fileManager.updateAcl(s1.getId(), Arrays.asList(Long.toString(data_d1_d2_d3)), externalUser, new File.FileAclParams
                (DENY_FILE_PERMISSIONS, AclParams.Action.SET, null), ownerSessionId);
        fileManager.updateAcl(s1.getId(), Arrays.asList(Long.toString(data_d1_d2_d3_d4_txt)), externalUser, new File.FileAclParams
                (ALL_FILE_PERMISSIONS, AclParams.Action.SET, null), ownerSessionId);

        smp1 = catalogManager.getSampleManager().create(s1.getId(), new Sample().setId("smp1"), QueryOptions.empty(), ownerSessionId).first();
        smp2 = catalogManager.getSampleManager().create(s1.getId(), new Sample().setId("smp2"), QueryOptions.empty(), ownerSessionId).first();
        smp3 = catalogManager.getSampleManager().create(s1.getId(), new Sample().setId("smp3"), QueryOptions.empty(), ownerSessionId).first();
        smp4 = catalogManager.getSampleManager().create(s1.getId(), new Sample().setId("smp4"), QueryOptions.empty(), ownerSessionId).first();
        smp5 = catalogManager.getSampleManager().create(s1.getId(), new Sample().setId("smp5"), QueryOptions.empty(), ownerSessionId).first();
        smp6 = catalogManager.getSampleManager().create(s1.getId(), new Sample().setId("smp6"), QueryOptions.empty(), ownerSessionId).first();

        catalogManager.getCohortManager().create(s1.getId(), new Cohort().setId("all").setSamples(Arrays.asList(smp1, smp2, smp3)),
                QueryOptions.empty(), ownerSessionId);

        ind1 = catalogManager.getIndividualManager().create(s1.getId(), new Individual().setId("ind1"), QueryOptions.empty(), ownerSessionId)
                .first().getUid();
        ind2 = catalogManager.getIndividualManager().create(s1.getId(), new Individual().setId("ind2"), QueryOptions.empty(), ownerSessionId)
                .first().getUid();

        catalogManager.getSampleManager().update(s1.getId(), smp1.getId(), new ObjectMap(SampleDBAdaptor.QueryParams.INDIVIDUAL.key(), ind1),
                QueryOptions.empty(), ownerSessionId);
        catalogManager.getSampleManager().update(s1.getId(), smp2.getId(), new ObjectMap(SampleDBAdaptor.QueryParams.INDIVIDUAL.key(), ind2),
                QueryOptions.empty(), ownerSessionId);

        catalogManager.getSampleManager().updateAcl(s1.getId(), Arrays.asList(Long.toString(smp1.getUid())), externalUser, allSamplePermissions,
                ownerSessionId);
        catalogManager.getSampleManager().updateAcl(s1.getId(), Arrays.asList(Long.toString(smp3.getUid())), externalUser, noSamplePermissions,
                ownerSessionId);
        catalogManager.getSampleManager().updateAcl(s1.getId(), Arrays.asList(Long.toString(smp2.getUid())), "*", noSamplePermissions,
                ownerSessionId);
        catalogManager.getSampleManager().updateAcl(s1.getId(), Arrays.asList(Long.toString(smp5.getUid())), externalUser, noSamplePermissions,
                ownerSessionId);
        catalogManager.getSampleManager().updateAcl(s1.getId(), Arrays.asList(Long.toString(smp6.getUid())), "@members", allSamplePermissions,
                ownerSessionId);
    }
    ...
    public void addMemberToGroup() throws CatalogException {
        updateGroup(s1.getId(), groupAdmin, externalUser, null, null, ownerSessionId);
    ...
    public void addMemberToGroupExistingNoPermission() throws CatalogException {
        ...
        updateGroup(s1.getId(), groupAdmin, externalUser, null, null, externalSessionId);
    }

    @Test
    public void changeGroupMembership() throws CatalogException {
        updateGroup(s1.getId(), groupAdmin, externalUser, null, null, ownerSessionId);
//        catalogManager.addUsersToGroup(s1.getId(), groupAdmin, externalUser, ownerSessionId);
        Map<String, Group> groups = getGroupMap();
        assertTrue(groups.get(groupAdmin).getUserIds().contains(externalUser));
//        thrown.expect(CatalogException.class);
//        catalogManager.addUsersToGroup(s1.getId(), groupMember, externalUser, ownerSessionId);
        updateGroup(s1.getId(), groupAdmin, null, externalUser, null, ownerSessionId);
        catalogManager.getStudyManager().createGroup(s1.getId(), groupMember, externalUser, ownerSessionId);
        //        catalogManager.updateGroup(s1.getId(), groupMember, externalUser, null, null, ownerSessionId);
        groups = getGroupMap();
...
//    }
    ...
    public void removeMemberFromGroup() throws CatalogException {
        // Create new group
        catalogManager.getStudyManager().createGroup(String.valueOf(s1.getId()), groupMember, studyAdminUser1 + "," + studyAdminUser2,
                ownerSessionId);

        // Remove one of the users
        updateGroup(s1.getId(), groupMember, null, studyAdminUser1, null, ownerSessionId);
        assertFalse(getGroupMap().get(groupMember).getUserIds().contains(studyAdminUser1));

        // Remove the last user in the admin group
        updateGroup(s1.getId(), groupMember, null, studyAdminUser2, null, ownerSessionId);
...
//        thrown.expectMessage("ACL defined");
        catalogManager.getStudyManager().deleteGroup(s1.getId(), groupMember, ownerSessionId);
    ...
    public void removeMemberFromGroupNoPermission() throws CatalogException {
        thrown.expect(CatalogAuthorizationException.class);
        updateGroup(s1.getId(), groupAdmin, null, ownerUser, null, externalSessionId);
//        catalogManager.removeUsersFromGroup(s1.getId(), groupAdmin, ownerUser, externalSessionId);
    }

    @Test
    public void removeMemberFromNonExistingGroup() throws CatalogException {
        thrown.expect(CatalogException.class);
//        catalogManager.removeUsersFromGroup(s1.getId(), "NO_GROUP", ownerUser, ownerSessionId);
        updateGroup(s1.getId(), "NO_GROUP", null, ownerUser, null, ownerSessionId);
    }

    @Test
    public void removeMemberFromNonBelongingGroup() throws CatalogException {
        thrown.expect(CatalogException.class);
//        catalogManager.removeUsersFromGroup(s1.getId(), groupMember, ownerUser, ownerSessionId);
        updateGroup(s1.getId(), groupMember, null, ownerUser, null, ownerSessionId);
    }
    ...
    public void addExistingUserToRole() throws CatalogException {
        ...
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), newUser, aclParams, ownerSessionId).get(0);
    }
    ...
    public void addExistingUserToRole2() throws CatalogException {
        ...
        Study.StudyAclParams aclParams = new Study.StudyAclParams("", AclParams.Action.ADD, AuthorizationManager.ROLE_ANALYST);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), newUser, aclParams, memberSessionId).get(0);
    }
...
//        catalogManager.addUsersToGroup(s1.getId(), group, newUser, studyAdmin1SessionId);
        catalogManager.getStudyManager().createGroup(s1.getId(), group, newUser, studyAdmin1SessionId);
        Study.StudyAclParams aclParams = new Study.StudyAclParams("", AclParams.Action.ADD, AuthorizationManager.ROLE_ANALYST);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), group, aclParams, studyAdmin1SessionId).get(0);
        QueryResult<StudyAclEntry> studyAcls = catalogManager.getAuthorizationManager().getStudyAcl(studyAdminUser1, s1.getUid(), group);
    ...
    public void addNonExistingUserToRole() throws CatalogException {
        ...
        Study.StudyAclParams aclParams = new Study.StudyAclParams("", AclParams.Action.ADD, AuthorizationManager.ROLE_ANALYST);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), userNotRegistered, aclParams, studyAdmin1SessionId).get(0);
    }
    ...
    public void addNonExistingGroupToRole() throws CatalogException {
        ...
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), groupNotRegistered, new Study.StudyAclParams("",
    ...
    public void changeUserRole() throws CatalogException {
        QueryResult<StudyAclEntry> studyAcls = catalogManager.getAuthorizationManager().getStudyAcl(studyAdminUser1, s1.getUid(),
                externalUser);
        assertEquals(1, studyAcls.getNumResults());
        ...
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), externalUser, aclParams1, studyAdmin1SessionId).get(0);
        Study.StudyAclParams aclParams = new Study.StudyAclParams("", AclParams.Action.ADD, AuthorizationManager.ROLE_ANALYST);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), externalUser, aclParams, studyAdmin1SessionId).get(0);
        String userId = catalogManager.getUserManager().getUserId(studyAdmin1SessionId);
        long studyId = catalogManager.getStudyManager().getId(userId, s1.getId());
...
//        catalogManager.unshareStudy(s1.getId(), externalUser, studyAdmin1SessionId);
    ...
    public void removeUserFromRole2() throws CatalogException {
        thrown.expect(CatalogAuthorizationException.class);
        Study.StudyAclParams aclParams = new Study.StudyAclParams(null, AclParams.Action.RESET, null);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), externalUser, aclParams, memberSessionId).get(0);
    }

    @Test
    public void removeGroupFromRole() throws CatalogException {
        String group = "@newGroup";
        catalogManager.getStudyManager().createGroup(s1.getId(), group, studyAdminUser1 + "," + studyAdminUser2,
                studyAdmin1SessionId);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), group, new Study.StudyAclParams("", AclParams.Action.SET, "admin"), ownerSessionId);

        long studyId1 = catalogManager.getStudyManager().getId(studyAdminUser1, s1.getId());
        ...
        Study.StudyAclParams aclParams = new Study.StudyAclParams(null, AclParams.Action.RESET, null);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), group, aclParams, ownerSessionId).get(0);
    ...
    public void removeNonExistingUserFromRole() throws CatalogException {
        ...
        thrown.expect(CatalogException.class);
        thrown.expectMessage("does not exist");
//        catalogManager.unshareStudy(s1.getId(), userNotRegistered, ownerSessionId);
        Study.StudyAclParams aclParams = new Study.StudyAclParams(null, AclParams.Action.RESET, null);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), userNotRegistered, aclParams, ownerSessionId).get(0);
    }
    ...
    public void removeNonExistingGroupFromRole() throws CatalogException {
        ...
        thrown.expectMessage("not found");
        Study.StudyAclParams aclParams = new Study.StudyAclParams(null, AclParams.Action.RESET, null);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), groupNotRegistered, aclParams, ownerSessionId).get(0);
    }
    ...
    public void readProjectDeny() throws CatalogException {
        catalogManager.getStudyManager().updateGroup(String.valueOf(s1.getId()), "@members", new GroupParams(externalUser,
    ...
    public void readStudy() throws CatalogException {
        QueryResult<Study> study = catalogManager.getStudyManager().get(s1.getId(), null, ownerSessionId);
        assertEquals(1, study.getNumResults());
        study = catalogManager.getStudyManager().get(s1.getId(), null, memberSessionId);
        assertEquals(1, study.getNumResults());
    }
    ...
    public void readStudyDeny() throws CatalogException {
        catalogManager.getStudyManager().updateGroup(String.valueOf(s1.getId()), "@members", new GroupParams(externalUser,
                GroupParams.Action.REMOVE), ownerSessionId);
        thrown.expect(CatalogAuthorizationException.class);
        catalogManager.getStudyManager().get(s1.getId(), null, externalSessionId);
    }
    ...
    public void readExplicitlyUnsharedFile() throws CatalogException {
        ...
        fileManager.updateAcl(s1.getId(), Arrays.asList(Long.toString(data_d1)), memberUser, new File.FileAclParams(null, AclParams.Action.SET,
                null), ownerSessionId);
        thrown.expect(CatalogAuthorizationException.class);
...
//        catalogManager.addUsersToGroup(s1.getId(), "@external", newUser, ownerSessionId);
        catalogManager.getStudyManager().createGroup(s1.getId(), newGroup, newUser, ownerSessionId);
        // Add the group to the locked role, so no permissions will be given
        Study.StudyAclParams aclParams = new Study.StudyAclParams("", AclParams.Action.ADD, AuthorizationManager.ROLE_LOCKED);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), newGroup, aclParams, ownerSessionId).get(0);
        // Specify all file permissions for that concrete file
        fileManager.updateAcl(s1.getId(), Arrays.asList(Long.toString(data_d1_d2_d3_d4)), newGroup, new File.FileAclParams(ALL_FILE_PERMISSIONS,
                AclParams.Action.SET, null), ownerSessionId);
    ...
    public void readFileForbiddenForUser() throws CatalogException {
        ...
        catalogManager.getStudyManager().createGroup(s1.getId(), groupMember, externalUser, ownerSessionId);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), groupMember, new Study.StudyAclParams("", AclParams.Action.SET,
                "admin"), ownerSessionId);
        fileManager.updateAcl(s1.getId(), Arrays.asList(Long.toString(data_d1_d2)), externalUser, new File.FileAclParams(DENY_FILE_PERMISSIONS,
                AclParams.Action.SET, null), ownerSessionId);
        thrown.expect(CatalogAuthorizationException.class);
...
//        catalogManager.addUsersToGroup(s1.getId(), "@external", newUser, ownerSessionId);
        ...
        Study.StudyAclParams aclParams = new Study.StudyAclParams("", AclParams.Action.ADD, AuthorizationManager.ROLE_LOCKED);
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), newGroup, aclParams, ownerSessionId).get(0);
        fileManager.updateAcl(s1.getId(), Arrays.asList(Long.toString(data_d1_d2)), newGroup, new File.FileAclParams(ALL_FILE_PERMISSIONS,
                AclParams.Action.SET, null), ownerSessionId);
    ...
    public void createFileByOwnerNoGroups() throws CatalogException {
        QueryResult<File> folder = catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/newFolder").toString(),
    ...
    public void createExplicitlySharedFile() throws CatalogException {
        catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/d1/folder/").toString(), null, false, null,
                QueryOptions.empty(), externalSessionId);
    }

    @Test
    public void createInheritedSharedFile() throws CatalogException {
        catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/d1/d2/folder/").toString(), null, false,
    ...
    public void createExplicitlyForbiddenFile() throws CatalogException {
        thrown.expect(CatalogAuthorizationException.class);
        catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/d1/d2/d3/folder/").toString(), null, false,
                null, QueryOptions.empty(), externalSessionId);
    }
    ...
    public void createInheritedForbiddenFile() throws CatalogException {
        thrown.expect(CatalogAuthorizationException.class);
        catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/d1/d2/d3/d4/folder/").toString(), null, false,
                null, QueryOptions.empty(), externalSessionId);
    }
    ...
    public void createNonSharedFile() throws CatalogException {
        thrown.expect(CatalogAuthorizationException.class);
        catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("folder/").toString(), null, false, null,
                QueryOptions.empty(), externalSessionId);
    }
    ...
    public void createFileNoStudyMember() throws CatalogException, IOException {
        ...
        String sessionId = catalogManager.getUserManager().login(newUser, password);
        thrown.expect(CatalogAuthorizationException.class);
        catalogManager.getFileManager().createFolder(s1.getId(), Paths.get("data/my_folder/").toString(), null, false, null,
    ...
    public void readSampleOwnerUser() throws CatalogException {
        QueryResult<Sample> sample = catalogManager.getSampleManager().get(smp1.getUid(), null, ownerSessionId);
        assertEquals(1, sample.getNumResults());
        sample = catalogManager.getSampleManager().get(smp2.getUid(), null, ownerSessionId);
        assertEquals(1, sample.getNumResults());
        sample = catalogManager.getSampleManager().get(smp3.getUid(), null, ownerSessionId);
        assertEquals(1, sample.getNumResults());
        ...
        catalogManager.getStudyManager().updateAcl(Arrays.asList(s1.getId()), ownerUser, aclParams, ownerSessionId).get(0);
        catalogManager.getSampleManager().updateAcl(s1.getId(), Arrays.asList(Long.toString(smp1.getUid())), ownerUser, noSamplePermissions, ownerSessionId);

        sample = catalogManager.getSampleManager().get(smp1.getUid(), null, ownerSessionId);
        assertEquals(1, sample.getNumResults());
    }

    @Test
    public void readSampleExplicitShared() throws CatalogException {
        QueryResult<Sample> sample = catalogManager.getSampleManager().get(smp1.getUid(), null, externalSessionId);
        assertEquals(1, sample.getNumResults());
    }
    ...
    public void readSampleExplicitUnshared() throws CatalogException {
        QueryResult<Sample> sample = catalogManager.getSampleManager().get(smp1.getUid(), null, externalSessionId);
        assertEquals(1, sample.getNumResults());
        catalogManager.getAuthorizationManager().removeAcls(Arrays.asList(smp1.getUid()), Arrays.asList(externalUser), null, Entity.SAMPLE);
        thrown.expect(CatalogAuthorizationException.class);
        catalogManager.getSampleManager().get(smp1.getUid(), null, externalSessionId);
    }
    ...
    public void readSampleExplicitForbidden() throws CatalogException {
        thrown.expect(CatalogAuthorizationException.class);
        catalogManager.getSampleManager().get(smp3.getUid(), null, externalSessionId);
    }
    ...
    public void readSampleExternalUser() throws CatalogException, IOException {
        ...
        String sessionId = catalogManager.getUserManager().login(newUser, password);
        thrown.expect(CatalogAuthorizationException.class);
        catalogManager.getSampleManager().get(smp2.getUid(), null, sessionId);
    }

    @Test
    public void readSampleAdminUser() throws CatalogException {
        QueryResult<Sample> sample = catalogManager.getSampleManager().get(smp1.getUid(), null, studyAdmin1SessionId);
        assertEquals(1, sample.getNumResults());
        sample = catalogManager.getSampleManager().get(smp3.getUid(), null, studyAdmin1SessionId);
        assertEquals(1, sample.getNumResults());
    }

    @Test
    public void readSampleForbiddenForExternalUser() throws CatalogException {
        catalogManager.getSampleManager().updateAcl(String.valueOf(s1.getId()), Arrays.asList(smp2.getId()), externalUser, new Sample.SampleAclParams("",
...
//        catalogManager.addUsersToGroup(s1.getId(), "@external", newUser, ownerSessionId);
        ...
        catalogManager.getSampleManager().updateAcl(s1.getId(), Arrays.asList(Long.toString(smp4.getUid())), newGroup, allSamplePermissions, ownerSessionId);

        QueryResult<Sample> sample = catalogManager.getSampleManager().get(smp4.getUid(), null, sessionId);
        assertEquals(1, sample.getNumResults());
    }

    @Test
    public void readSampleSharedForOthers() throws CatalogException {
        QueryResult<Sample> sample = catalogManager.getSampleManager().get(smp6.getUid(), null, externalSessionId);
        assertEquals(1, sample.getNumResults());
    }
    ...
    public void readSampleSharedForOthersNotWithStudyPermissions() throws CatalogException, IOException {
        ...
        catalogManager.getUserManager().create(newUser, newUser, "asda@mail.com", password, "org", 1000L, Account.FULL, null, null);
        String sessionId = catalogManager.getUserManager().login(ownerUser, password);
        catalogManager.getStudyManager().updateGroup(s1.getId(), "@members", new GroupParams(newUser, GroupParams.Action.ADD),
                ownerSessionId);

        QueryResult<Sample> sample = catalogManager.getSampleManager().get(smp6.getUid(), null, sessionId);
        assertEquals(1, sample.getNumResults());
    }

    @Test
    public void adminShareSampleWithOtherUser() throws CatalogException {
        catalogManager.getSampleManager().updateAcl(s1.getId(), Arrays.asList(Long.toString(smp4.getUid())), externalUser, allSamplePermissions, studyAdmin1SessionId);
        QueryResult<Sample> sample = catalogManager.getSampleManager().get(smp4.getUid(), null, externalSessionId);
        assertEquals(1, sample.getNumResults());
    }

    @Test
    public void readAllSamplesOwner() throws CatalogException {
        Map<Long, Sample> sampleMap = catalogManager.getSampleManager().get(s1.getId(), new Query(), new QueryOptions(), ownerSessionId)
...
//        Map<Long, Sample> sampleMap = catalogManager.getSampleManager().get(s1.getId(), new Query(), new QueryOptions(), studyAdmin1SessionId)
...
//    }
    ...
    public void readAllSamplesMember() throws CatalogException {
        Map<Long, Sample> sampleMap = catalogManager.getSampleManager().get(s1.getId(), new Query(), new QueryOptions(), externalSessionId)
                .getResult().stream().collect(Collectors.toMap(Sample::getUid, f -> f));
    ...
    public void readCohort() throws CatalogException {
        assertEquals(1, catalogManager.getCohortManager().get(s1.getId(), new Query(), null, ownerSessionId).getNumResults());
        assertEquals(0, catalogManager.getCohortManager().get(s1.getId(), new Query(), null, externalSessionId).getNumResults());
    }
    ...
    public void readAllIndividuals() throws CatalogException {
        assertEquals(2, catalogManager.getIndividualManager().get(s1.getId(), new Query(), null, ownerSessionId).getNumResults());
        assertEquals(2, catalogManager.getIndividualManager().get(s1.getId(), new Query(), null, studyAdmin1SessionId).getNumResults());
        assertEquals(0, catalogManager.getIndividualManager().get(s1.getId(), new Query(), null, externalSessionId).getNumResults());
    }
    ...
    public void getAllJobs() throws CatalogException {
        Job job = new Job()
                .setName("job1")
                .setOutDir(new File().setUid(data_d1_d2));
        long job1 = catalogManager.getJobManager().create(s1.getId(), job, null, ownerSessionId).first().getUid();

        job.setName("job2");
        long job2 = catalogManager.getJobManager().create(s1.getId(), job, null, ownerSessionId).first().getUid();

        job.setName("job3");
        long job3 = catalogManager.getJobManager().create(s1.getId(), job, null, ownerSessionId).first().getUid();

        job.setName("job4");
        long job4 = catalogManager.getJobManager().create(s1.getId(), job, null, ownerSessionId).first().getUid();
    ...
    private void checkGetAllJobs(Collection<Long> expectedJobs, String sessionId) throws CatalogException {
        QueryResult<Job> allJobs = catalogManager.getJobManager().get(s1.getId(), (Query) null, null, sessionId);
    ...
    private Map<String, Group> getGroupMap() throws CatalogException {
        return catalogManager.getStudyManager().get(s1.getId(), null, ownerSessionId).first().getGroups()
                .stream()
