...
public class FamilyManagerTest extends GenericTest {
    ...
    public void deleteFamilyTest() throws CatalogException {
        ...
        OpenCGAResult<Individual> result = catalogManager.getIndividualManager().get(organizationId, STUDY, members, QueryOptions.empty(), sessionIdUser);
    ...
    public void deleteWithClinicalAnalysisTest() throws CatalogException {
        ...
        catalogManager.getIndividualManager().create(organizationId, STUDY, individual, Arrays.asList("sample1", "sample2"), QueryOptions.empty(),
                sessionIdUser);

        individual = new Individual().setId("father");
        catalogManager.getIndividualManager().create(organizationId, STUDY, individual, Arrays.asList("sample3"), QueryOptions.empty(), sessionIdUser);

        Family family = new Family().setId("family");
        catalogManager.getFamilyManager().create(organizationId, STUDY, family, Arrays.asList("proband", "father"), QueryOptions.empty(), sessionIdUser);
        ...
        OpenCGAResult<?> result = catalogManager.getFamilyManager().get(organizationId, STUDY, family.getId(), QueryOptions.empty(), sessionIdUser);
    ...
    public void updateFamilyReferencesInIndividualTest() throws CatalogException {
        ...
        familyManager.update(organizationId, STUDY, familyDataResult.first().getId(), updateParams, QueryOptions.empty(), sessionIdUser);
        Family family = familyManager.get(organizationId, STUDY, familyDataResult.first().getId(), QueryOptions.empty(), sessionIdUser).first();
        ...
        List<Individual> individualList = catalogManager.getIndividualManager().get(organizationId, STUDY, Arrays.asList("child2", "child3"),
        ...
        individualList = catalogManager.getIndividualManager().get(organizationId, STUDY, Arrays.asList("john", "father", "mother", "child1", "child2",
    ...
    public void updateFamilyRoles() throws CatalogException {
        ...
        catalogManager.getIndividualManager().update(organizationId, STUDY, father.getId(), updateParams, QueryOptions.empty(), sessionIdUser);
        ...
        catalogManager.getIndividualManager().update(organizationId, STUDY, mother.getId(), updateParams, QueryOptions.empty(), sessionIdUser);

        updateParams = new IndividualUpdateParams()
                .setFather(new IndividualReferenceParam().setId(father.getId()))
                .setMother(new IndividualReferenceParam().setId(mother.getId()));
        catalogManager.getIndividualManager().update(organizationId, STUDY, proband.getId(), updateParams, QueryOptions.empty(), sessionIdUser);
        catalogManager.getIndividualManager().update(organizationId, STUDY, brother.getId(), updateParams, QueryOptions.empty(), sessionIdUser);
        catalogManager.getIndividualManager().update(organizationId, STUDY, sister.getId(), updateParams, QueryOptions.empty(), sessionIdUser);
        catalogManager.getIndividualManager().update(organizationId, STUDY, sibling.getId(), updateParams, QueryOptions.empty(), sessionIdUser);
...
//        new QueryOptions(ParamConstants.FAMILY_UPDATE_ROLES_PARAM, true), sessionIdUser);
        ...
        family = catalogManager.getFamilyManager().get(organizationId, STUDY, "family", QueryOptions.empty(), sessionIdUser);
    ...
    public void testPropagateFamilyPermission() throws CatalogException {
        ...
        try {
            familyManager.get(organizationId, STUDY, "Martinez-Martinez", QueryOptions.empty(), token);
        ...
        familyManager.updateAcl(organizationId, STUDY, new FamilyAclParams("VIEW", "Martinez-Martinez", null, null, FamilyAclParams.Propagate.NO), "user2",
                ParamUtils.AclAction.SET, sessionIdUser);
        DataResult<Family> familyDataResult = familyManager.get(organizationId, STUDY, "Martinez-Martinez", QueryOptions.empty(), token);
        assertEquals(1, familyDataResult.getNumResults());
        ...
        familyManager.updateAcl(organizationId, STUDY, new FamilyAclParams("VIEW", "Martinez-Martinez", null, null, FamilyAclParams.Propagate.YES), "user2",
                ParamUtils.AclAction.SET, sessionIdUser);
        familyDataResult = familyManager.get(organizationId, STUDY, "Martinez-Martinez", QueryOptions.empty(), token);
        assertEquals(1, familyDataResult.getNumResults());
        assertEquals(5, familyDataResult.first().getMembers().size());
        ...
        familyManager.updateAcl(organizationId, STUDY, new FamilyAclParams("VIEW", "Martinez-Martinez", null, null,
    ...
    public void getFamilyWithOnlyAllowedMembers2() throws CatalogException, IOException {
        ...
        catalogManager.getIndividualManager().updateAcl(organizationId, STUDY, Collections.singletonList("child2"), "user2",
        ...
        catalogManager.getIndividualManager().updateAcl(organizationId, STUDY, Collections.singletonList("child3"), "user2",
                new IndividualAclParams("", "VIEW"), ParamUtils.AclAction.SET, false, sessionIdUser);
        familyDataResult = familyManager.get(organizationId, STUDY, "Martinez-Martinez", QueryOptions.empty(), token);
        assertEquals(1, familyDataResult.getNumResults());
        ...
        familyDataResult = familyManager.get(organizationId, STUDY, "Martinez-Martinez",
    ...
    public void testUpdateWithLockedClinicalAnalysis() throws CatalogException {
        ...
        Family familyResult = catalogManager.getFamilyManager().get(organizationId, STUDY, "family", QueryOptions.empty(), sessionIdUser).first();
        ...
        ClinicalAnalysis clinicalResult = catalogManager.getClinicalAnalysisManager().get(organizationId, STUDY, "clinical", QueryOptions.empty(),
                sessionIdUser).first();
        ...
        clinicalResult = catalogManager.getClinicalAnalysisManager().get(organizationId, STUDY, "clinical2", QueryOptions.empty(), sessionIdUser).first();
        assertEquals(2, clinicalResult.getProband().getVersion());
        assertEquals(2, clinicalResult.getProband().getSamples().get(0).getVersion());  // sample1 version
        assertEquals(2, clinicalResult.getFamily().getVersion());
        ...
        catalogManager.getClinicalAnalysisManager().update(organizationId, STUDY, "clinical", new ClinicalAnalysisUpdateParams().setLocked(true),
                QueryOptions.empty(), sessionIdUser);
        clinicalResult = catalogManager.getClinicalAnalysisManager().get(organizationId, STUDY, "clinical", QueryOptions.empty(), sessionIdUser).first();
        assertTrue(clinicalResult.isLocked());

        // Update family with version increment
        catalogManager.getFamilyManager().update(organizationId, STUDY, "family", new FamilyUpdateParams().setName("bl"), new QueryOptions(),
                sessionIdUser);

        familyResult = catalogManager.getFamilyManager().get(organizationId, STUDY, "family", QueryOptions.empty(), sessionIdUser).first();
    ...
    public void includeMemberIdOnly() throws CatalogException {
        ...
        DataResult<Family> family = familyManager.get(organizationId, STUDY, "family", options, sessionIdUser);
    ...
    * */
    ...
    private DataResult<Family> createDummyFamily(String familyName, boolean createMissingMembers) throws CatalogException {
        ...
        OpenCGAResult<Family> familyOpenCGAResult = familyManager.create(organizationId, STUDY, family, memberIds, INCLUDE_RESULT, sessionIdUser);
    ...
    public void updateFamilyDisordersWhenIndividualDisorderIsUpdated() throws CatalogException {
        ...
        catalogManager.getIndividualManager().update(organizationId, STUDY, "child1", params, new QueryOptions(), sessionIdUser);
        DataResult<Individual> child1 = catalogManager.getIndividualManager().get(organizationId, STUDY, "child1", QueryOptions.empty(), sessionIdUser);
        ...
        catalogManager.getIndividualManager().update(organizationId, STUDY, "child1", params, queryOptions, sessionIdUser);
        child1 = catalogManager.getIndividualManager().get(organizationId, STUDY, "child1", QueryOptions.empty(), sessionIdUser);
    ...
    public void disordersDistinctTest() throws CatalogException {
        ...
        catalogManager.getIndividualManager().update(organizationId, STUDY, "child1", params1, new QueryOptions(), sessionIdUser);
        catalogManager.getIndividualManager().update(organizationId, STUDY, "child2", params2, new QueryOptions(), sessionIdUser);

        OpenCGAResult<?> distinct = catalogManager.getFamilyManager().distinct(organizationId, STUDY, "disorders.name", new Query(), sessionIdUser);
...
//    }
    ...
    public void updateFamilyMissingMember() throws CatalogException {
        ...
        Family family = familyManager.update(organizationId, STUDY, originalFamily.first().getId(), updateParams,
    ...
    public void recalculateRoles() throws CatalogException {
        ...
        assertEquals(1, familyManager.update(organizationId, STUDY, originalFamily.first().getId(), updateParams, options, sessionIdUser).getNumUpdated());
    }
    ...
    public void updateFamilyQualityControl() throws CatalogException {
        ...
        DataResult<Family> updatedFamily = familyManager.update(organizationId, STUDY, originalFamily.first().getId(),
                updateParams, QueryOptions.empty(), sessionIdUser);
        assertEquals(1, updatedFamily.getNumUpdated());

        updatedFamily = familyManager.get(organizationId, STUDY, originalFamily.first().getId(), QueryOptions.empty(), sessionIdUser);
    ...
    public void incrementVersionTest() throws CatalogException {
        ...
        catalogManager.getFamilyManager().create(organizationId, STUDY, dummyFamily1, members, QueryOptions.empty(), sessionIdUser);
        dummyFamily2.setMembers(null);
        catalogManager.getFamilyManager().create(organizationId, STUDY, dummyFamily2, members, QueryOptions.empty(), sessionIdUser);

        OpenCGAResult<Family> result = catalogManager.getFamilyManager().get(organizationId, STUDY, Arrays.asList(dummyFamily1.getId(),
        ...
        catalogManager.getFamilyManager().update(organizationId, STUDY, dummyFamily1.getId(), new FamilyUpdateParams().setName("name"),
                QueryOptions.empty(), sessionIdUser);
        catalogManager.getFamilyManager().update(organizationId, STUDY, dummyFamily1.getId(), new FamilyUpdateParams().setName("name22"),
                QueryOptions.empty(), sessionIdUser);
        result = catalogManager.getFamilyManager().get(organizationId, STUDY, Arrays.asList(dummyFamily1.getId(), dummyFamily2.getId()),
                QueryOptions.empty(), sessionIdUser);
        assertEquals(2, result.getNumResults());
    ...
    public void memberReferenceTest() throws CatalogException {
        ...
        // Update family id
        catalogManager.getFamilyManager().update(organizationId, STUDY, dummyFamily1.getId(), new FamilyUpdateParams().setId("newId"), QueryOptions.empty(),
                sessionIdUser);
    ...
    public void updateInUseInCATest() throws CatalogException {
        ...
        catalogManager.getFamilyManager().create(organizationId, STUDY, family, members, QueryOptions.empty(), sessionIdUser);
        ...
        // Update family id
        catalogManager.getFamilyManager().update(organizationId, STUDY, family.getId(), new FamilyUpdateParams().setId("newId"), QueryOptions.empty(),
                sessionIdUser);

        OpenCGAResult<ClinicalAnalysis> result = catalogManager.getClinicalAnalysisManager().get(organizationId, STUDY,
    ...
    public void updateFamilyMembers() throws CatalogException {
        ...
        family.setMembers(null);
        catalogManager.getFamilyManager().create(organizationId, STUDY, family, Collections.singletonList(child.getId()), QueryOptions.empty(),
                sessionIdUser);
