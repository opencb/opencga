...
//    }
    ...
    public void getEffectivePermissionsNoAdmins() throws CatalogException {
        ...
        catalogManager.getAdminManager().getEffectivePermissions(studyFqn, Arrays.asList(s_1, s_2), Enums.Resource.SAMPLE.name(),
                sessionIdUser2);
    }
    ...
    public void getEffectivePermissionsMissingEntries() throws CatalogException {
        ...
        catalogManager.getAdminManager().getEffectivePermissions(studyFqn, Collections.emptyList(), "sampl", token);
    }
    ...
    public void getEffectivePermissionsWrongCategory() throws CatalogException {
        ...
        catalogManager.getAdminManager().getEffectivePermissions(studyFqn, Collections.singletonList(s_1), "sampl", token);
    }
    ...
    public void getEffectivePermissionsWrongPermission() throws CatalogException {
        ...
        catalogManager.getAdminManager().getEffectivePermissions(studyFqn, Collections.singletonList(s_1), Collections.singletonList("VIE"),
                Enums.Resource.SAMPLE.name(), token);
    }

    @Test
    public void getEffectivePermissions() throws CatalogException {
        OpenCGAResult<Acl> aclList = catalogManager.getAdminManager().getEffectivePermissions(studyFqn, Collections.singletonList(s_1),
                Collections.singletonList(SamplePermissions.VIEW.name()), Enums.Resource.SAMPLE.name(), token);
        assertEquals(1, aclList.getNumResults());
        assertEquals(s_1, aclList.first().getId());
        ...
        aclList = catalogManager.getAdminManager().getEffectivePermissions(studyFqn, Collections.singletonList(s_1),
                Enums.Resource.SAMPLE.name(), token);
        assertEquals(1, aclList.getNumResults());
        assertEquals(s_1, aclList.first().getId());
    ...
    private void generatePermissionScenario() throws CatalogException {
         ...
         */
        catalogManager.getUserManager().create("user4", "User Name", "mail@ebi.ac.uk", TestParamConstants.PASSWORD, "", null, Account.AccountType.FULL, opencgaToken);
        catalogManager.getUserManager().create("user5", "User2 Name", "mail2@ebi.ac.uk", TestParamConstants.PASSWORD, "", null, Account.AccountType.FULL, opencgaToken);
        catalogManager.getUserManager().create("user6", "User3 Name", "user.2@e.mail", TestParamConstants.PASSWORD, "ACME", null, Account.AccountType.FULL, opencgaToken);
        catalogManager.getUserManager().create("user7", "User3 Name", "user.2@e.mail", TestParamConstants.PASSWORD, "ACME", null, Account.AccountType.FULL, opencgaToken);

        catalogManager.getStudyManager().createGroup(studyFqn, "@view", Arrays.asList("user2", "user7"), token);
        catalogManager.getStudyManager().createGroup(studyFqn, "@write", Collections.singletonList("user3"), token);

        catalogManager.getStudyManager().updateAcl(studyFqn, "@view,user5", new StudyAclParams(null, "view_only"), ParamUtils.AclAction.SET, token);
        catalogManager.getStudyManager().updateAcl(studyFqn, "@write,user6", new StudyAclParams(null, "analyst"), ParamUtils.AclAction.SET, token);
        catalogManager.getStudyManager().updateAcl(studyFqn, "user4,user7", new StudyAclParams(null, null), ParamUtils.AclAction.SET, token);

        catalogManager.getSampleManager().updateAcl(studyFqn, Arrays.asList(s_1, s_2), "@view", new SampleAclParams(null, null, null, null, "WRITE"), ParamUtils.AclAction.SET, token);
        catalogManager.getSampleManager().updateAcl(studyFqn, Arrays.asList(s_1, s_2), "@write", new SampleAclParams(null, null, null, null, null), ParamUtils.AclAction.SET, token);
        catalogManager.getSampleManager().updateAcl(studyFqn, Arrays.asList(s_1, s_2), "user4", new SampleAclParams(null, null, null, null, "VIEW"), ParamUtils.AclAction.SET, token);
        catalogManager.getSampleManager().updateAcl(studyFqn, Collections.singletonList(s_1), "user3", new SampleAclParams(null, null, null, null, "VIEW"), ParamUtils.AclAction.SET, token);
    }
}