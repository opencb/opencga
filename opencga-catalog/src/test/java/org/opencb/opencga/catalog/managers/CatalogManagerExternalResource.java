...
public class CatalogManagerExternalResource extends ExternalResource {
    ...
    public void before() throws Exception {
        ...
        // Start embedded MongoDB if enabled
        EmbeddedMongoDBManager embeddedMongo = EmbeddedMongoDBManager.getInstance();
        if (!embeddedMongo.isRunning()) {
            embeddedMongo.start();
            System.out.println("Embedded MongoDB started on port: " + embeddedMongo.getPort());
        }
    ...
    public Path clearOpenCGAHome(String testName) throws IOException {
        ...
        // Update MongoDB connection string to use embedded MongoDB if enabled
        EmbeddedMongoDBManager embeddedMongo = EmbeddedMongoDBManager.getInstance();
        if (embeddedMongo.isEnabled()) {
            configuration.getCatalog().getDatabase().setHosts(java.util.Collections.singletonList(embeddedMongo.getConnectionString()));
            System.out.println("Using embedded MongoDB at: " + embeddedMongo.getConnectionString());
        }
    ...
    public void after() {
        ...
        // Note: We don't stop embedded MongoDB here as it's shared across test classes
        // It will be stopped when the JVM exits or via shutdown hook
