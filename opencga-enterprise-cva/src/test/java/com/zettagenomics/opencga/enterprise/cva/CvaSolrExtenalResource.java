...
public class CvaSolrExtenalResource extends ExternalResource {
    ...
    public CvaSolrExtenalResource() {
        this(true);
    }

    public CvaSolrExtenalResource(boolean embeded) {
...
//        Path rootDir = getTmpRootDir();
        ...
        if (embeded) {
            solrClient = create(solrHome, rootDir.resolve("configsets").toString(),
                    ClinicalVariantSolrEngine.INTERPRETATIONS_COLLECTION
                            + "," + ClinicalVariantSolrEngine.CLINICAL_VARIANTS_COLLECTION
                            + "," + ClinicalVariantSolrEngine.CLINICAL_VARIANT_EVIDENCES_COLLECTION);
        } else {
...
//            }
    ...
    public ClinicalVariantSolrEngine configure() {
        ClinicalVariantSolrEngine cvaEngine = new ClinicalVariantSolrEngine();
    ...
    public static URI getResourceUri(String resourceName, String targetName, Path rootDir) throws IOException {
        ...
        if (!resourcePath.toFile().exists()) {
            InputStream stream = ClinicalVariantSolrEngineTest.class.getClassLoader().getResourceAsStream(resourceName);
