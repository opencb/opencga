...
public class OperationsVariantStorageCommandOptions {
        ...
        public VariantAnnotationExtensionConfigureCommandOptions variantAnnotationExtensionConfigureCommandOptions;
    ...
    public OperationsVariantStorageCommandOptions(CommonCommandOptions commonCommandOptions, JCommander jCommander) {
        ...
        this.variantAnnotationExtensionConfigureCommandOptions = new VariantAnnotationExtensionConfigureCommandOptions();
    ...
    public class AggregateVariantFamilyCommandOptions {
        ...
        @Parameter(names = {"--family"}, description = "Family ID to select the samples to aggregate. Exclusively used when 'samples' is not set.", required = false, arity = 1)
        public String family;
    
        @Parameter(names = {"--samples"}, description = "Samples within the same family to aggregate. Other family members not provided won't be added automatically. Exclusively used when 'family' is not set.", required = false, arity = 1)
    ...
    public class LauncherVariantIndexCommandOptions {
        ...
        @Parameter(names = {"--index-params-transform"}, description = "If present it only runs the transform stage, no load is executed", required = false, help = true, arity = 0)
        public boolean indexParamsTransform = false;
    
        @Parameter(names = {"--index-params-gvcf"}, description = "Hint to indicate that the input file is in gVCF format.", required = false, help = true, arity = 0)
        public boolean indexParamsGvcf = false;
    
        @Parameter(names = {"--index-params-normalization-skip"}, description = "Do not execute the normalization process. WARN: INDELs will be stored with the context base", required = false, help = true, arity = 0)
        public boolean indexParamsNormalizationSkip = false;
    
        @Parameter(names = {"--index-params-reference-genome"}, description = "Reference genome in FASTA format used during the normalization step for a complete left alignment", required = false, arity = 1)
        ...
        @Parameter(names = {"--index-params-load"}, description = "If present only the load stage is executed, transformation is skipped", required = false, help = true, arity = 0)
        public boolean indexParamsLoad = false;
    
        @Parameter(names = {"--index-params-force-reload"}, description = "If the file is already loaded, force a file reload", required = false, help = true, arity = 0)
        public boolean indexParamsForceReload = false;
    
        @Parameter(names = {"--index-params-load-split-data"}, description = "Indicate that the variants from a group of samples is split in multiple files, either by CHROMOSOME or by REGION. In either case, variants from different files must not overlap.", required = false, arity = 1)
        public String indexParamsLoadSplitData;
    
        @Parameter(names = {"--index-params-load-multi-file-data"}, description = "Indicate the presence of multiple files for the same sample. Each file could be the result of a different vcf-caller or experiment over the same sample.", required = false, help = true, arity = 0)
        public boolean indexParamsLoadMultiFileData = false;
    
        @Parameter(names = {"--index-params-load-sample-index"}, description = "Build sample index while loading. (yes, no, auto) [auto]", required = false, arity = 1)
        public String indexParamsLoadSampleIndex;
    
        @Parameter(names = {"--index-params-load-archive"}, description = "Load archive data. (yes, no, auto) [auto]", required = false, arity = 1)
        public String indexParamsLoadArchive;
    
        @Parameter(names = {"--index-params-load-hom-ref"}, description = "Load HOM_REF genotypes. (yes, no, auto) [auto]", required = false, arity = 1)
        public String indexParamsLoadHomRef;
    
        @Parameter(names = {"--index-params-post-load-check"}, description = "Execute post load checks over the database. (yes, no, auto) [auto]", required = false, arity = 1)
        public String indexParamsPostLoadCheck;
    
        @Parameter(names = {"--index-params-include-genotypes"}, description = "Load the genotype data for the current file. This only applies to the GT field from the FORMAT. All the rest of fields from the INFO and FORMAT will be loaded. Use this parameter skip load data when the GT field is not reliable, or its only value across the file is './.'. If 'auto', genotypes will be automatically excluded if all genotypes are either missing, ./. or 0/0. (yes, no, auto) [auto]", required = false, arity = 1)
        public String indexParamsIncludeGenotypes;
    
        @Parameter(names = {"--index-params-include-sample-data"}, description = "Index including other sample data fields (i.e. FORMAT fields). Use 'all', 'none', or CSV with the fields to load.", required = false, arity = 1)
        public String indexParamsIncludeSampleData;
    
        @Parameter(names = {"--index-params-merge"}, description = "Currently two levels of merge are supported: 'basic' mode merge genotypes of the same variants while 'advanced' merge multiallelic and overlapping variants.", required = false, arity = 1)
        public String indexParamsMerge;
    
        @Parameter(names = {"--index-params-deduplication-policy"}, description = "Specify how duplicated variants should be handled. Available policies: 'discard', 'maxQual'", required = false, arity = 1)
        public String indexParamsDeduplicationPolicy;
    
        @Parameter(names = {"--index-params-calculate-stats"}, description = "Calculate indexed variants statistics after the load step", required = false, help = true, arity = 0)
        public boolean indexParamsCalculateStats = false;
    
        @Parameter(names = {"--index-params-aggregated"}, description = "Select the type of aggregated VCF file: none, basic, EVS or ExAC", required = false, arity = 1)
        public String indexParamsAggregated;
    
        @Parameter(names = {"--index-params-aggregation-mapping-file"}, description = "File containing population names mapping in an aggregated VCF file", required = false, arity = 1)
        public String indexParamsAggregationMappingFile;
    
        @Parameter(names = {"--index-params-annotate"}, description = "Annotate indexed variants after the load step", required = false, help = true, arity = 0)
        public boolean indexParamsAnnotate = false;
    
        @Parameter(names = {"--index-params-annotator"}, description = "Annotation source {cellbase_rest, cellbase_db_adaptor}", required = false, arity = 1)
        public String indexParamsAnnotator;
    
        @Parameter(names = {"--index-params-overwrite-annotations"}, description = "Overwrite annotations in variants already present", required = false, help = true, arity = 0)
        public boolean indexParamsOverwriteAnnotations = false;
    
        @Parameter(names = {"--index-params-index-search"}, description = "Add files to the secondary search index", required = false, help = true, arity = 0)
        public boolean indexParamsIndexSearch = false;
    
        @Parameter(names = {"--index-params-skip-indexed-files"}, description = "Do not fail if any of the input files was already indexed.", required = false, help = true, arity = 0)
